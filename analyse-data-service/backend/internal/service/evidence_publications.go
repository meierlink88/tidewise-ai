...
func evidencePublicationError(err error) error {
	...
	if errors.As(err, &validation) {
		return publicErrorWithDetails(v1.StatusUnprocessableEntity, "EVIDENCE_PUBLICATION_INVALID", "Evidence Publication failed validation", map[string]any{"issues": validation.Issues})
	}
...
func rawEvidenceInput(input v1.RawEvidence) evidencepublication.RawEvidence {
	return evidencepublication.RawEvidence{
		...
		SourceLevel: input.SourceLevel, SourceURL: input.SourceURL, IsOriginal: input.IsOriginal,
...
func evidenceInput(input v1.AtomicEvidence) evidencepublication.Evidence {
	return evidencepublication.Evidence{
		EvidenceID: input.EvidenceID, SplitOrder: input.SplitOrder, LayerType: input.LayerType,
