...
type tagPlan struct {
	record      PublicationEventTag
	disposition Disposition
}

func (s *Service) Import(ctx context.Context, callerSubject string, publication Publication) (Result, error) {
...
func planPublication(publication Publication) ([]*rawPlan, []*eventPlan, []string) {
	...
	for _, input := range publication.Events {
		...
		plan := &eventPlan{
			...
			record: PublicationEvent{
				...
				OccurredAt: input.OccurredAt, FactPayload: model.FactPayload(input.FactPayload),
				FirstSeenAt: firstSeenAt, KnowableAt: knowableAt,
				EventStatus: model.EventStatusConfirmed, FactStatus: model.FactStatusVerified,
			},
			disposition: DispositionCreated,
		}
...
func observationTimes(event Event, rawByArtifact map[string]*rawPlan) (time.Time, time.Time) {
	...
	for eventIndex, plan := range eventPlans {
		...
		for tagIndex, input := range plan.input.Tags {
			...
			record := PublicationEventTag{
				...
				ReviewStatus: model.ReviewStatusApproved, Confidence: string(input.Confidence),
...
func sameRawDocument(left, right PublicationRawDocument) bool {
...
func sameEventCore(left, right PublicationEvent) bool {
	return left.Title == right.Title &&
		...
		left.EventStatus == model.EventStatusConfirmed &&
		left.FactStatus == model.FactStatusVerified
}
...
func sameEventTag(left, right PublicationEventTag) bool {
	return left.AssignSource == right.AssignSource &&
		left.ReviewStatus == model.ReviewStatusApproved &&
