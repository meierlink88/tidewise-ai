...
func TestFrozenAdditiveChainNodeRelationManifestCombinesAcceptedBaselineExactly(t *testing.T) {
	...
	if counts[model.ChainNodeRelationSubcategoryOf] != 108 || counts[model.ChainNodeRelationComponentOf] != 3 || counts[model.ChainNodeRelationInputTo] != 93 || counts[model.ChainNodeRelationDependsOn] != 8 {
...
func TestFrozenChainNodeRelationManifestRejectsCountAndTypeDrift(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "type count", mutate: func(frozen *frozenChainNodeRelationManifest, _ *ChainNodeRelationManifest) {
			frozen.ByRelationType[model.ChainNodeRelationInputTo] = 2
		}},
...
func frozenChainNodeRelationManifestPath(t *testing.T) string {
	t.Helper()
	return filepath.Join("..", "..", "..", "data", "entity_foundation", "relationships", "reviewed_chain_node_relations", "approved-candidate-manifest.json")
}

func frozenAdditiveChainNodeRelationManifestPath(t *testing.T) string {
	t.Helper()
	return filepath.Join("..", "..", "..", "data", "entity_foundation", "relationships", "reviewed_chain_node_relations", "additive-final-candidate-manifest.json")
}
