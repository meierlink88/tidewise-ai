...
func TestSectorConvergenceManifestRejectsInvalidTargetsAndVersions(t *testing.T) {
	...
	cases := map[string]SectorConvergenceManifest{
		...
		"sector action with index": mutateConvergenceManifest(valid, func(m *SectorConvergenceManifest) {
			...
			m.Convergences[0].TargetEntityType = model.EntityTypeIndex
		}),
		"retire with target": mutateConvergenceManifest(valid, func(m *SectorConvergenceManifest) {
			for i := range m.Convergences {
				if m.Convergences[i].Action == SectorConvergenceRetireWithoutCanonical {
					...
					m.Convergences[i].TargetEntityType = model.EntityTypeSector
					break
				}
			}
		}),
...
func TestMemoryCorrectionDriftRollsBackAndEdgeConflictsResolveDeterministically(t *testing.T) {
	relationships := map[string]Relationship{
		...
		"relationship:a": {Key: "relationship:a", From: "market:a_share", To: "sector:target", RelationType: "covers_sector", Status: model.StatusActive},
	}
	resolveMemoryRelationshipConflicts(relationships)
	if relationships["relationship:a"].Status != model.StatusActive || relationships["relationship:z"].Status != model.StatusInactive {
	...
	drifted.Status = model.StatusActive
...
func TestMemoryIndexTargetRedirectsPolicyCompatibleEdgeAndNoTargetDeactivates(t *testing.T) {
	repo := NewMemoryRepository()
	seedMemoryConvergenceEntities(t, repo)
	market := Entity{Key: "market:test", EntityType: model.EntityTypeMarket, LayerCode: "market", Name: "测试市场", CanonicalName: "测试市场", Status: model.StatusActive}
	...
	repo.relationships["relationship:index_redirect"] = Relationship{Key: "relationship:index_redirect", From: "market:test", To: indexLegacy, RelationType: "tracks_index", SourceName: "review", SourceURL: manifest.ReviewSourceURL, VerifiedAt: verifiedAt, Status: model.StatusActive}
	repo.relationships["relationship:no_target"] = Relationship{Key: "relationship:no_target", From: "market:test", To: noTargetLegacy, RelationType: "tracks_index", SourceName: "review", SourceURL: manifest.ReviewSourceURL, VerifiedAt: verifiedAt, Status: model.StatusActive}
	...
	if redirected.To == indexLegacy || redirected.Status != model.StatusActive {
		t.Fatalf("index relationship = %+v", redirected)
	}
	if target := repo.entities[redirected.To]; target.EntityType != model.EntityTypeIndex {
		t.Fatalf("redirect target = %+v", target)
	}
	if got := repo.relationships["relationship:no_target"].Status; got != model.StatusInactive {
...
func TestMemoryIndexTargetBlocksSectorOnlyMapping(t *testing.T) {
	...
	if repo.ConvergenceAuditCount() != before || repo.entities[legacy].Status != model.StatusActive {
...
func TestPostgresConvergenceRelationshipPlanRedirectsIndexAndDeactivatesNoTarget(t *testing.T) {
	...
	relationship := Relationship{Key: "relationship:test", From: "market:test", To: "sector:legacy", RelationType: "tracks_index", SourceName: "review", SourceURL: "https://example.com/review", VerifiedAt: verifiedAt, Status: model.StatusActive}
	entities := map[string]Entity{"market:test": {Key: "market:test", EntityType: model.EntityTypeMarket}, "sector:legacy": {Key: "sector:legacy", EntityType: model.EntityTypeSector}, "index:csi300": {Key: "index:csi300", EntityType: model.EntityTypeIndex}}
	redirected, disposition, err := planConvergenceRelationship(relationship, entities, "sector:legacy", "index:csi300")
	if err != nil {
		t.Fatal(err)
	}
	if disposition != convergenceEdgeRedirect || redirected.To != "index:csi300" || redirected.Status != model.StatusActive {
	...
	if disposition != convergenceEdgeDeactivate || retired.Status != model.StatusInactive {
...
func TestConvergenceRelationshipTransitionMatrix(t *testing.T) {
	...
	type transition struct {
		...
		fromType, toType                     model.EntityType
		initialStatus                        model.Status
		wantDisposition                      convergenceEdgeDisposition
	}
	cases := []transition{
		{name: "sector-to-sector", relation: "covers_sector", fromTarget: "sector:old", toTarget: "sector:new", fromType: model.EntityTypeSector, toType: model.EntityTypeSector, initialStatus: model.StatusActive, wantDisposition: convergenceEdgeRedirect},
		{name: "sector-to-index", relation: "tracks_index", fromTarget: "sector:old", toTarget: "index:new", fromType: model.EntityTypeSector, toType: model.EntityTypeIndex, initialStatus: model.StatusActive, wantDisposition: convergenceEdgeRedirect},
		{name: "index-to-index", relation: "tracks_index", fromTarget: "index:old", toTarget: "index:new", fromType: model.EntityTypeIndex, toType: model.EntityTypeIndex, initialStatus: model.StatusActive, wantDisposition: convergenceEdgeRedirect},
		{name: "index-to-sector", relation: "covers_sector", fromTarget: "index:old", toTarget: "sector:new", fromType: model.EntityTypeIndex, toType: model.EntityTypeSector, initialStatus: model.StatusActive, wantDisposition: convergenceEdgeRedirect},
		{name: "target-to-no-target", relation: "covers_sector", fromTarget: "sector:old", toTarget: "", fromType: model.EntityTypeSector, initialStatus: model.StatusActive, wantDisposition: convergenceEdgeDeactivate},
		{name: "no-target-to-sector", relation: "covers_sector", fromTarget: "sector:legacy", toTarget: "sector:new", fromType: model.EntityTypeSector, toType: model.EntityTypeSector, initialStatus: model.StatusInactive, wantDisposition: convergenceEdgeRedirect},
		{name: "no-target-to-index", relation: "tracks_index", fromTarget: "sector:legacy", toTarget: "index:new", fromType: model.EntityTypeSector, toType: model.EntityTypeIndex, initialStatus: model.StatusInactive, wantDisposition: convergenceEdgeRedirect},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			entities := map[string]Entity{"market:test": {Key: "market:test", EntityType: model.EntityTypeMarket}, tc.fromTarget: {Key: tc.fromTarget, EntityType: tc.fromType}}
			if tc.toTarget != "" {
			...
			if tc.toTarget == "" {
				if planned.Status != model.StatusInactive {
					t.Fatalf("status = %q", planned.Status)
				}
			} else if planned.To != tc.toTarget || planned.Status != model.StatusActive {
...
func TestConvergenceRelationshipTransitionRejectsIncompatiblePolicy(t *testing.T) {
	verifiedAt := reviewedConvergenceFixture(t).ReviewedAt
	rel := Relationship{Key: "relationship:test", From: "market:test", To: "sector:old", RelationType: "covers_sector", SourceName: "review", SourceURL: "https://example.com/review", VerifiedAt: verifiedAt, Status: model.StatusActive}
	entities := map[string]Entity{"market:test": {Key: "market:test", EntityType: model.EntityTypeMarket}, "sector:old": {Key: "sector:old", EntityType: model.EntityTypeSector}, "index:new": {Key: "index:new", EntityType: model.EntityTypeIndex}}
...
func TestMemoryCorrectionUsesRecordedRowsForNoTargetAndIndexSwap(t *testing.T) {
	...
	for key, to := range map[string]string{"relationship:no_target": noTargetLegacy, "relationship:index": indexLegacy} {
		repo.relationships[key] = Relationship{Key: key, From: "market:test", To: to, RelationType: "tracks_index", SourceName: "review", SourceURL: manifest.ReviewSourceURL, VerifiedAt: manifest.ReviewedAt, Status: model.StatusActive}
	}
	service := NewService(repo)
	if _, err := service.ApplySectorConvergence(context.Background(), Manifest{}, manifest, SectorConvergenceModeInitial); err != nil {
		t.Fatal(err)
	}
	if repo.relationships["relationship:no_target"].Status != model.StatusInactive {
	...
	correction := mutateConvergenceManifest(manifest, func(m *SectorConvergenceManifest) {
		...
		m.Convergences[noTargetIndex].TargetEntityType = model.EntityTypeIndex
	...
	if restored.Status != model.StatusActive || restored.To != indexKey {
		t.Fatalf("restored edge = %+v", restored)
	}
	if repo.relationships["relationship:index"].Status != model.StatusInactive {
...
func TestNormalSeedFailsClosedWithActiveLegacySectors(t *testing.T) {
	repo := NewMemoryRepository()
	legacy := Entity{Key: "sector:ths_concept_ai", EntityType: model.EntityTypeSector, LayerCode: "sector", Name: "人工智能", CanonicalName: "人工智能", Status: model.StatusActive, Profile: []byte(`{"sector_system":"ths","sector_type":"concept"}`)}
	if _, err := repo.UpsertEntity(context.Background(), legacy); err != nil {
		t.Fatal(err)
	}
	service := NewService(repo)
	_, err := service.Apply(context.Background(), Manifest{Entities: []Entity{{Key: "sector:theme_artificial_intelligence", EntityType: model.EntityTypeSector, LayerCode: "sector", Name: "人工智能", CanonicalName: "人工智能", Aliases: []string{"Artificial Intelligence"}, Profile: []byte(`{"sector_system":"canonical","sector_type":"theme"}`)}}}, ApplyOptions{})
...
func TestReviewedConvergenceClosesCanonicalSeedCountsInMemory(t *testing.T) {
	...
	manifest := reviewedConvergenceFixture(t)
	for _, item := range manifest.Convergences {
		legacy := Entity{Key: item.LegacyEntityKey, EntityType: model.EntityTypeSector, LayerCode: "sector", Name: item.LegacyName, CanonicalName: item.LegacyName, Status: model.StatusActive, Profile: []byte(`{"sector_system":"ths","sector_type":"concept"}`)}
		if _, err := repo.UpsertEntity(context.Background(), legacy); err != nil {
			t.Fatal(err)
		}
	}
	...
	for key, entity := range repo.entities {
		if entity.EntityType != model.EntityTypeSector {
			continue
		}
		if strings.HasPrefix(key, "sector:ths_") && entity.Status == model.StatusInactive {
			inactiveLegacy++
		}
		if !strings.HasPrefix(key, "sector:ths_") && entity.Status == model.StatusActive {
			activeCanonical++
		}
	}
...
func TestMemoryOrdinarySeedPreservesCurrentConvergenceAliasesOnly(t *testing.T) {
	...
	for _, item := range manifest.Convergences {
		if item.TargetEntityType == model.EntityTypeSector {
			count := 0
...
func seedMemoryConvergenceEntities(t *testing.T, repo *MemoryRepository) {
	...
	manifest := reviewedConvergenceFixture(t)
	for _, item := range manifest.Convergences {
		entity := Entity{Key: item.LegacyEntityKey, EntityType: model.EntityTypeSector, LayerCode: "sector", Name: item.LegacyName, CanonicalName: item.LegacyName, Status: model.StatusActive, Profile: []byte(`{"sector_system":"ths","sector_type":"concept"}`)}
		if _, err := repo.UpsertEntity(context.Background(), entity); err != nil {
			t.Fatal(err)
		}
		if item.TargetEntityKey != "" {
			target := Entity{Key: item.TargetEntityKey, EntityType: item.TargetEntityType, LayerCode: string(item.TargetEntityType), Name: "目标实体", CanonicalName: "目标实体", Status: model.StatusActive}
			if item.TargetEntityType == model.EntityTypeSector {
