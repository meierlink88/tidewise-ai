...
func TestProductionManifestRejectsRetiredModels(t *testing.T) {
	tests := []Manifest{
		{Entities: []Entity{{Key: "sector:test", EntityType: model.EntityTypeSector}}},
		{Entities: []Entity{{Key: "industry_chain:test", EntityType: model.EntityTypeIndustryChain}}},
...
func TestMinimalChainNodeAndThemeProfiles(t *testing.T) {
	for _, tc := range []struct {
		...
		{model.EntityTypeChainNode, `{"definition":"节点定义","boundary_note":null}`},
		{model.EntityTypeTheme, `{"definition":"投研视角","boundary_note":"不等同于节点"}`},
	} {
		if err := validateProfileData(tc.entityType, []byte(tc.data)); err != nil {
			t.Fatalf("validateProfileData(%q) error = %v", tc.entityType, err)
		}
	}
	if fields := requiredProfileFields(model.EntityTypeTheme); strings.Join(fields, ",") != "definition,boundary_note" {
...
func TestProductionChainNodeRejectsLegacyOrMissingProfileFields(t *testing.T) {
	for _, profile := range []string{
		...
		manifest := Manifest{Entities: []Entity{{Key: "chain_node:test", EntityType: model.EntityTypeChainNode, Profile: []byte(profile)}}}
		if err := ValidateProductionManifest(manifest); err == nil {
