...
func TestMemoryRepositoryUpsertsEntityIdempotently(t *testing.T) {
	...
	entity := Entity{
		Key:           "economy:cn",
		EntityType:    model.EntityTypeEconomy,
		LayerCode:     "economy",
		Name:          "中国",
		CanonicalName: "中国",
...
func TestMemoryRepositoryUpsertsProfilesAndRelationshipsIdempotently(t *testing.T) {
	...
	alliance := Entity{
		...
		EntityType:    model.EntityTypeAllianceOrg,
		LayerCode:     "alliance",
		Name:          "二十国集团",
		CanonicalName: "二十国集团",
		Status:        model.StatusActive,
...
func TestMemoryRepositoryUpdatesRelationshipProvenance(t *testing.T) {
	...
	for _, entity := range []Entity{
		{Key: "economy:cn", EntityType: model.EntityTypeEconomy, LayerCode: "economy", Name: "中国", CanonicalName: "中国"},
		{Key: "alliance_org:g20", EntityType: model.EntityTypeAllianceOrg, LayerCode: "alliance", Name: "二十国集团", CanonicalName: "二十国集团"},
	} {
		if _, err := repo.UpsertEntity(context.Background(), entity); err != nil {
			t.Fatalf("UpsertEntity() error = %v", err)
		}
...
func TestMemoryRepositoryUpdatesLatestSectorSourceMappingSnapshotIdempotently(t *testing.T) {
	repo := NewMemoryRepository()
	entity := Entity{Key: "sector:theme_ai", EntityType: model.EntityTypeSector, LayerCode: "sector", Name: "人工智能", CanonicalName: "人工智能", Aliases: []string{"Artificial Intelligence"}}
	if _, err := repo.UpsertEntity(context.Background(), entity); err != nil {
		t.Fatalf("UpsertEntity() error = %v", err)
	}
...
func TestMemoryRepositoryUpdatesReviewFieldsWithoutReplacingNewerSnapshot(t *testing.T) {
	repo := NewMemoryRepository()
	for _, entity := range []Entity{
		{Key: "sector:theme_ai", EntityType: model.EntityTypeSector, LayerCode: "sector", Name: "人工智能", CanonicalName: "人工智能", Aliases: []string{"Artificial Intelligence"}},
		{Key: "sector:theme_ai_merged", EntityType: model.EntityTypeSector, LayerCode: "sector", Name: "人工智能产业", CanonicalName: "人工智能产业", Aliases: []string{"Artificial Intelligence Industry"}},
	} {
		if _, err := repo.UpsertEntity(context.Background(), entity); err != nil {
			t.Fatalf("UpsertEntity() error = %v", err)
		}
