...
func TestServiceAppliesManifestInOrderAndReportsStats(t *testing.T) {
	...
	manifest := Manifest{
		Entities: []Entity{
			testSeedEntity("economy:cn", model.EntityTypeEconomy, "economy", `{"country_code":"CN","currency_code":"CNY"}`),
			testSeedEntity("alliance_org:g20", model.EntityTypeAllianceOrg, "alliance", `{"org_code":"G20","org_type":"economic_forum"}`),
		},
		Relationships: []Relationship{
			{Key: "relationship:cn_member_of_g20", From: "economy:cn", To: "alliance_org:g20", RelationType: "member_of", Status: model.StatusActive},
		},
	}
	...
	if report.ByEntityType[model.EntityTypeEconomy] != 1 || report.ByLayerCode["alliance"] != 1 {
...
func TestServiceReportsBenchmarkProfileCounts(t *testing.T) {
	...
	manifest := Manifest{
		Entities: []Entity{
			testSeedEntity("benchmark:us_10y_treasury_yield", model.EntityTypeBenchmark, "benchmark", `{
	...
	if first.ByEntityType[model.EntityTypeBenchmark] != 1 || first.ProfileCounts["benchmark_profiles"] != 1 {
...
func TestServiceSkipsInactiveEntitiesByDefault(t *testing.T) {
	repo := &recordingRepository{}
	service := NewService(repo)
	inactive := testSeedEntity("economy:inactive", model.EntityTypeEconomy, "economy", `{"country_code":"ZZ","currency_code":"ZZZ"}`)
	...
	manifest := Manifest{
		...
		Relationships: []Relationship{
			{Key: "relationship:skip_inactive", From: "economy:cn", To: "economy:inactive", RelationType: "related_to", Status: model.StatusActive},
		},
	}
...
func (r *recordingRepository) UpsertExternalIdentifier(_ context.Context, identifier model.EntityExternalIdentifier) (WriteResult, error) {
...
func testSeedEntity(key string, entityType model.EntityType, layerCode string, profile string) Entity {
	name := key
	aliases := []string(nil)
	if entityType == model.EntityTypeBenchmark {
		name = "测试基准"
