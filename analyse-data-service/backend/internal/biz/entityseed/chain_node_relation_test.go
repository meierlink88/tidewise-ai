...
func TestPostgresRelationDryRunRejectsPhysicalConstraintsBeforeDatabaseAccess(t *testing.T) {
	...
	defer db.Close()
	manifest := ChainNodeRelationManifest{PhysicalConstraints: []model.ChainNodePhysicalConstraint{{ID: "constraint"}}}
...
func TestChainNodeRelationDryRunReportsCreateUnchangedUpdateAndConflict(t *testing.T) {
	...
	base := model.ChainNodeRelation{ID: "r1", FromChainNodeEntityID: "a", ToChainNodeEntityID: "b", RelationType: model.ChainNodeRelationSubcategoryOf, Mechanism: "定义范围从属", EvidenceNote: "定义与边界直接支持", Provenance: "approved-node-manifest", VerifiedAt: now, Status: model.StatusActive}
	repo := newChainNodeRelationMemoryRepository([]string{"a", "b", "c"})
	report, err := DryRunChainNodeRelations(context.Background(), repo, []model.ChainNodeRelation{base})
	...
	report, err = DryRunChainNodeRelations(context.Background(), repo, []model.ChainNodeRelation{updated})
	if err != nil || report.Updated != 1 {
		t.Fatalf("updated report = %+v, %v", report, err)
	}
	conflict := base
	conflict.FromChainNodeEntityID = "c"
	if _, err := DryRunChainNodeRelations(context.Background(), repo, []model.ChainNodeRelation{conflict}); err == nil || !strings.Contains(err.Error(), "conflict") {
		t.Fatalf("conflict error = %v", err)
	}
	tupleConflict := base
	tupleConflict.ID = "r2"
	if _, err := DryRunChainNodeRelations(context.Background(), repo, []model.ChainNodeRelation{tupleConflict}); err == nil || !strings.Contains(err.Error(), "tuple conflict") {
		t.Fatalf("tuple conflict error = %v", err)
	}
}

func TestChainNodeRelationDryRunRejectsInactiveOrMissingEndpoint(t *testing.T) {
	relation := model.ChainNodeRelation{ID: "r", FromChainNodeEntityID: "a", ToChainNodeEntityID: "missing", RelationType: model.ChainNodeRelationInputTo, Mechanism: "直接投入", EvidenceNote: "证据", Provenance: "review", VerifiedAt: time.Now(), Status: model.StatusActive}
	_, err := DryRunChainNodeRelations(context.Background(), newChainNodeRelationMemoryRepository([]string{"a"}), []model.ChainNodeRelation{relation})
...
func TestPostgresChainNodeRelationDryRunUsesRepeatableReadOnlySnapshot(t *testing.T) {
	...
	defer db.Close()
	relation := model.ChainNodeRelation{ID: "r", FromChainNodeEntityID: "a", ToChainNodeEntityID: "b", RelationType: model.ChainNodeRelationSubcategoryOf, Mechanism: "定义范围从属", EvidenceNote: "定义证据", Provenance: "review", VerifiedAt: time.Date(2026, 7, 14, 0, 0, 0, 0, time.UTC), Status: model.StatusActive}
	mock.ExpectBegin()
	...
	mock.ExpectQuery("FROM chain_node_relations WHERE from_chain_node_entity_id").WithArgs("a", "b", model.ChainNodeRelationSubcategoryOf).WillReturnError(sql.ErrNoRows)
	mock.ExpectCommit()
