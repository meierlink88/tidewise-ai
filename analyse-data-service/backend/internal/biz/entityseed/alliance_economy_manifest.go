...
func validateApprovedAlliances(items []ApprovedAlliance) (map[string]struct{}, error) {
	...
	for _, item := range items {
		...
		profile := model.AllianceOrgProfile{EntityID: item.EntityKey, Abbreviation: item.Profile.Abbreviation, LeadershipSummary: item.Profile.LeadershipSummary, InfluenceScopeSummary: item.Profile.InfluenceScopeSummary}
