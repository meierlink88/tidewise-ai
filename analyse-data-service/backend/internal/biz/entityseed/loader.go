...
func Validate(manifest Manifest) error {
	...
	for _, profile := range manifest.Profiles {
		...
		if entityType == model.EntityTypeSector {
	...
	for _, mapping := range manifest.SectorSourceMappings {
		...
		if !ok || entity.EntityType != model.EntityTypeSector {
	...
	for _, relationship := range manifest.Relationships {
		...
		if relationship.Status != "" && relationship.Status != model.StatusActive && relationship.Status != model.StatusInactive {
...
func validateEntity(entity Entity) error {
	...
	if entity.EntityType == model.EntityTypeSector && !strings.HasPrefix(entity.Key, "sector:ths_") {
...
func validateProfileData(entityType model.EntityType, data json.RawMessage) error {
	...
	if entityType == model.EntityTypeIndustryChain {
...
func validateSectorProfileFields(fields map[string]json.RawMessage) error {
	if raw, ok := fields["classification_code"]; ok && string(raw) != "null" {
		var value model.SectorClassification
	...
	if raw, ok := fields["review_status"]; ok && string(raw) != "null" {
		var value model.SectorReviewStatus
...
func validateSectorProfileReferences(data json.RawMessage, entityKeys map[string]Entity) error {
	...
	for _, reference := range []struct {
		...
		{"primary_market_entity_id", model.EntityTypeMarket},
		{"primary_economy_entity_id", model.EntityTypeEconomy},
	} {
...
func validSectorClassification(value model.SectorClassification) bool {
	switch value {
	case model.SectorClassificationIndustry, model.SectorClassificationTheme, model.SectorClassificationMarket, model.SectorClassificationStyle, model.SectorClassificationRegion:
		return true
	default:
		return false
	}
...
func validSectorReviewStatus(value model.SectorReviewStatus) bool {
	switch value {
	case model.SectorReviewCandidate, model.SectorReviewApproved, model.SectorReviewRejected:
		return true
	default:
		return false
	}
...
func normalizeSectorSourceMapping(mapping SectorSourceMapping) SectorSourceMapping {
	...
	if mapping.MappingStatus == "" {
		mapping.MappingStatus = string(model.SectorSourceMappingCandidate)
	}
...
func validateSectorSourceMapping(mapping SectorSourceMapping) error {
	...
	switch model.SectorSourceTaxonomyType(mapping.SourceTaxonomyType) {
	case model.SectorSourceTaxonomyConcept, model.SectorSourceTaxonomyIndustry, model.SectorSourceTaxonomyIndexSector:
	default:
		return fmt.Errorf("unsupported source taxonomy type %q", mapping.SourceTaxonomyType)
	}
	switch model.SectorSourceMappingStatus(mapping.MappingStatus) {
	case model.SectorSourceMappingCandidate, model.SectorSourceMappingApproved, model.SectorSourceMappingRejected, model.SectorSourceMappingMerged:
	default:
...
func normalizeDefaults(manifest *Manifest) {
	for i := range manifest.Entities {
		if manifest.Entities[i].Status == "" {
			manifest.Entities[i].Status = model.StatusActive
		}
	}
	for i := range manifest.Relationships {
		if manifest.Relationships[i].Status == "" {
			manifest.Relationships[i].Status = model.StatusActive
		}
	}
