...
func TestBuildFirstBatchDryRunReportsNodeDriftAndExactIdempotency(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "wrong type conflict", existing: withNodeType(wanted, model.EntityTypeTheme), wantAction: "conflict", wantReady: false},
		{name: "inactive conflict", existing: withNodeStatus(wanted, model.StatusInactive), wantAction: "conflict", wantReady: false},
		{name: "merged conflict", existing: withNodeStatus(wanted, model.Status("merged")), wantAction: "conflict", wantReady: false},
	} {
...
func TestBuildFirstBatchDryRunReportsMappingSnapshotActionsAndConflicts(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "status drift updated", snapshot: externalSnapshot(withMappingStatus(wantedMapping, model.StatusInactive)), wantAction: string(WriteUpdated), wantReady: true},
...
func withNodeType(identity FirstBatchIdentity, entityType model.EntityType) FirstBatchIdentity {
	identity.EntityType = entityType
	return identity
}
func withNodeStatus(identity FirstBatchIdentity, status model.Status) FirstBatchIdentity {
...
func firstBatchMappingSnapshot(mapping FirstBatchMappingDraft, entityID string) FirstBatchExternalIdentifier {
	...
	return FirstBatchExternalIdentifier{ID: externalIdentifierSeedUUID(identity), EntityID: entityID, SourceSystem: mapping.SourceSystem, SourceTaxonomyType: mapping.SourceTaxonomyType, ExternalCode: mapping.ExternalCode, ExternalName: mapping.ExternalName, Status: model.StatusActive}
}
...
func withMappingStatus(identifier FirstBatchExternalIdentifier, status model.Status) FirstBatchExternalIdentifier {
