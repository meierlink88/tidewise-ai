...
func (r PostgresRepository) ApplySectorConvergence(ctx context.Context, entities Manifest, manifest SectorConvergenceManifest, mode SectorConvergenceMode) (SectorConvergenceReport, error) {
	...
	for _, item := range manifest.Convergences {
		...
		if item.TargetEntityKey != "" {
			...
			var targetType model.EntityType
		...
		if mode == SectorConvergenceModeCorrection && status != model.StatusInactive {
			return SectorConvergenceReport{}, fmt.Errorf("correction drift for legacy sector %q", item.LegacyEntityKey)
		}
		auditID := bizidentity.NormalizeUUID("entity_convergence", fmt.Sprintf("%s|%d", item.LegacyEntityKey, manifest.ManifestVersion))
		...
		if mode == SectorConvergenceModeCorrection {
			previousID := bizidentity.NormalizeUUID("entity_convergence", fmt.Sprintf("%s|%d", item.LegacyEntityKey, *manifest.PreviousManifestVersion))
	...
	for _, item := range manifest.Convergences {
		...
		for _, reference := range refs {
			...
			if rule, known := registry.Rule(reference.table, reference.column); known {
				...
				if exists && item.TargetEntityType != model.EntityTypeSector && rule.SectorOnly {
...
func moveSectorReferences(ctx context.Context, tx *sql.Tx, convergenceID, legacyID, targetID, alias string) (sectorMoveCounts, error) {
	...
	aliasMoveID := bizidentity.NormalizeUUID("entity_convergence_alias_move", convergenceID+"|"+alias)
...
ORDER BY e.id FOR UPDATE OF e`, legacyID)
	...
	for _, item := range edges {
		...
		entities := map[string]Entity{item.fromKey: {Key: item.fromKey, EntityType: item.fromType}, item.toKey: {Key: item.toKey, EntityType: item.toType}, targetKey: {Key: targetKey, EntityType: model.EntityTypeIndex}}
...
func insertReferenceMove(ctx context.Context, tx *sql.Tx, convergenceID, table, column, rowID, fromID string, toID any, operation string) error {
	id := bizidentity.NormalizeUUID("entity_convergence_reference_move", convergenceID+"|"+table+"|"+column+"|"+rowID)
...
func applyRecordedCorrectionMoves(ctx context.Context, tx *sql.Tx, previousID, currentID string, targetType model.EntityType, targetID any, targetKey string) (sectorMoveCounts, error) {
	var counts sectorMoveCounts
	...
	if previousTargetID.Valid != previousTargetType.Valid || (previousTargetType.Valid && previousTargetType.String != string(model.EntityTypeSector) && previousTargetType.String != string(model.EntityTypeIndex)) {
	...
	for _, m := range moves {
		...
		if m.table == "sector_source_mappings" && m.column == "sector_entity_id" {
			...
			if targetType == model.EntityTypeSector {
...
func applyRecordedEdgeTransition(ctx context.Context, tx *sql.Tx, currentID, table, column, rowID, legacyID string, recordedToID, previousTargetID sql.NullString, targetType model.EntityType, targetID any, targetKey string) error {
	...
	if expectedStatus == model.StatusInactive {
