...
func TestDefaultSeedPathsExcludeRetiredMarketSectorFiles(t *testing.T) {
	paths := DefaultSeedPaths("seed-root")
	joined := strings.Join(paths, "\n")
	for _, forbidden := range []string{"sector_source_mappings.json", "covers_sector.json", "tracked_by_benchmark.json"} {
		if strings.Contains(joined, forbidden) {
			t.Fatalf("DefaultSeedPaths() includes retired path %q", forbidden)
		}
	}
}
...
func TestReviewedBenchmarkSeedFixtures(t *testing.T) {
	...
	for _, key := range benchmarkKeys {
		...
		if entity.EntityType != model.EntityTypeBenchmark {
	...
	if got := countEntitiesByType(manifest.Entities, model.EntityTypeBenchmark); got != 10 {
...
func TestValidateBenchmarkRequiresBilingualSearchNames(t *testing.T) {
	base := Entity{
		Key:           "benchmark:test",
		EntityType:    model.EntityTypeBenchmark,
		LayerCode:     "benchmark",
		Name:          "测试基准",
		CanonicalName: "测试基准",
...
func countEntitiesByType(entities []Entity, entityType model.EntityType) int {
	count := 0
...
func TestValidateRelationshipPolicyAcceptsSupportedTypeDirections(t *testing.T) {
	...
	entities := map[string]Entity{
		...
		"alliance_org:g20":    {Key: "alliance_org:g20", EntityType: model.EntityTypeAllianceOrg},
		"market:sse":          {Key: "market:sse", EntityType: model.EntityTypeMarket},
		"market:us_treasury":  {Key: "market:us_treasury", EntityType: model.EntityTypeMarket},
		"index:sse_composite": {Key: "index:sse_composite", EntityType: model.EntityTypeIndex},
		"benchmark:us_10y":    {Key: "benchmark:us_10y", EntityType: model.EntityTypeBenchmark},
		"company:test":        {Key: "company:test", EntityType: model.EntityTypeCompany},
		"security:test":       {Key: "security:test", EntityType: model.EntityTypeSecurity},
		"chain_node:test":     {Key: "chain_node:test", EntityType: model.EntityTypeChainNode},
		"person:test":         {Key: "person:test", EntityType: model.EntityTypePerson},
		"policy_body:test":    {Key: "policy_body:test", EntityType: model.EntityTypePolicyBody},
		"metric:test":         {Key: "metric:test", EntityType: model.EntityTypeMetric},
		"instrument:test":     {Key: "instrument:test", EntityType: model.EntityTypeInstrument},
...
func TestValidateMarketSectorRelationshipPolicies(t *testing.T) {
	...
	entities := map[string]Entity{
		"market:a_share":        {Key: "market:a_share", EntityType: model.EntityTypeMarket},
		"sector:industry_banks": {Key: "sector:industry_banks", EntityType: model.EntityTypeSector},
		"benchmark:bank_index":  {Key: "benchmark:bank_index", EntityType: model.EntityTypeBenchmark},
	}
	valid := []Relationship{
...
func TestLoadManifestAcceptsBenchmarkProfileWithNullableOfficialSeriesCode(t *testing.T) {
	...
	if manifest.Entities[0].EntityType != model.EntityTypeBenchmark {
...
func relationshipTestManifest() Manifest {
	...
	return Manifest{
		Entities: []Entity{
			{Key: "economy:cn", EntityType: model.EntityTypeEconomy, LayerCode: "economy", Name: "中国", CanonicalName: "中国", Profile: []byte(`{"country_code":"CN","currency_code":"CNY"}`)},
			{Key: "alliance_org:g20", EntityType: model.EntityTypeAllianceOrg, LayerCode: "alliance", Name: "二十国集团", CanonicalName: "二十国集团", Profile: []byte(`{"org_code":"G20","org_type":"economic_forum"}`)},
		},
