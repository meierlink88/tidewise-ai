...
type Repository interface {
	...
	UpsertExternalIdentifier(context.Context, model.EntityExternalIdentifier) (WriteResult, error)
...
type MemoryRepository struct {
	...
	externalIdentifiers              map[string]model.EntityExternalIdentifier
...
func (r *MemoryRepository) UpsertExternalIdentifier(_ context.Context, identifier model.EntityExternalIdentifier) (WriteResult, error) {
	...
	for key, entity := range r.entities {
		if entitySeedUUID(key) == identifier.EntityID && entity.EntityType == model.EntityTypeChainNode && entity.Status == model.StatusActive {
			targetFound = true
			break
		}
	}
...
func (r *MemoryRepository) HasRetiredIndustryEntities(_ context.Context) (bool, error) {
	...
	for _, entity := range r.entities {
		if entity.EntityType == model.EntityTypeSector || entity.EntityType == model.EntityTypeIndustryChain {
			return true, nil
		}
	}
	return false, nil
...
func (r *MemoryRepository) ApplySectorConvergence(_ context.Context, seedManifest Manifest, manifest SectorConvergenceManifest, mode SectorConvergenceMode) (SectorConvergenceReport, error) {
	...
	ownedAliases := cloneStringSliceMap(r.convergenceOwnedAliases)
	...
	for _, item := range manifest.Convergences {
		...
		if !ok || legacy.EntityType != model.EntityTypeSector {
		...
		if mode == SectorConvergenceModeCorrection && legacy.Status != model.StatusInactive {
		...
		if mode == SectorConvergenceModeCorrection {
			...
			if previous.TargetEntityType == model.EntityTypeSector && previous.TargetEntityKey != item.TargetEntityKey {
				if owned := ownedAliases[previous.TargetEntityKey]; owned != nil {
					ownedAliases[previous.TargetEntityKey] = removeAlias(owned, previous.LegacyName)
		...
		legacy.Status = model.StatusInactive
		entities[item.LegacyEntityKey] = legacy
		if item.TargetEntityType == model.EntityTypeSector {
			if !containsAlias(ownedAliases[item.TargetEntityKey], item.LegacyName) {
				ownedAliases[item.TargetEntityKey] = append(ownedAliases[item.TargetEntityKey], item.LegacyName)
			}
		...
		} else if item.TargetEntityType == model.EntityTypeIndex {
			for key, relationship := range relationships {
				if mode == SectorConvergenceModeCorrection {
					continue
				}
			...
			for key, relationship := range relationships {
				...
				if relationship.From == item.LegacyEntityKey || relationship.To == item.LegacyEntityKey {
					...
					if relationship.Status != model.StatusInactive {
						report.ReferencesMoved++
					}
					relationships[key] = planned
					relationshipMoves[auditKey] = append(relationshipMoves[auditKey], key)
...
func resolveMemoryRelationshipConflicts(relationships map[string]Relationship) {
	...
	for _, key := range keys {
		...
		if _, exists := seen[identity]; exists {
			relationship.Status = model.StatusInactive
			relationships[key] = relationship
			continue
		}
...
func (r *MemoryRepository) UpsertEntity(_ context.Context, entity Entity) (WriteResult, error) {
	...
	r.mu.Lock()
	defer r.mu.Unlock()
	entity.Aliases = mergeSeedAndOwnedAliases(entity.Aliases, r.convergenceOwnedAliases[entity.Key])
...
func mergeSeedAndOwnedAliases(seed, owned []string) []string {
	seen := map[string]struct{}{}
	result := make([]string, 0, len(seed)+len(owned))
	for _, group := range [][]string{seed, owned} {
...
func (r *MemoryRepository) UpsertRelationshipBatch(_ context.Context, relationships []Relationship) ([]WriteResult, error) {
	...
	for _, relationship := range relationships {
		if relationship.Status == "" {
			relationship.Status = model.StatusActive
		}
		from, fromOK := r.entities[relationship.From]
		to, toOK := r.entities[relationship.To]
		if !fromOK || !toOK || from.Status != model.StatusActive || to.Status != model.StatusActive {
