...
func ValidateProductionManifest(manifest Manifest) error {
	for _, entity := range manifest.Entities {
		...
		if entity.EntityType == model.EntityTypeChainNode || entity.EntityType == model.EntityTypeTheme {
	...
	for _, profile := range manifest.Profiles {
		if profile.EntityType == model.EntityTypeChainNode || profile.EntityType == model.EntityTypeTheme {
...
func validateUnifiedProfile(entityType model.EntityType, data []byte) error {
	...
	if entityType == model.EntityTypeChainNode {
