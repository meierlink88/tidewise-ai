...
func (s *Service) Import(ctx context.Context, publisherSubject string, publication Publication) (Result, error) {
	...
	err = s.store.InResearchAnchorImportTransaction(ctx, func(tx Transaction) error {
		...
		for _, anchor := range publication.Anchors {
			...
			if err := tx.InsertResearchAnchor(ctx, AnchorRecord{
			...
			for _, event := range anchor.Events {
				if err := tx.InsertResearchAnchorEvent(ctx, EventRecord{
			...
			for index, node := range anchor.PathNodes {
				if err := tx.InsertResearchAnchorPathNode(ctx, PathNodeRecord{
...
func buildPlan(publication Publication) (plan, error) {
	...
	for _, anchor := range publication.Anchors {
		anchorIDs[anchor.CenterChainNodeID] = AnchorID(publication.ThemeID, anchor.CenterChainNodeID)
		counts.EventAssociations += len(anchor.Events)
		counts.PathNodes += len(anchor.PathNodes)
	}
	return plan{
		ReceiptID:   identity.NormalizeUUID("research_anchor_import_receipt", publication.ThemeID),
		PayloadHash: payloadHash, AnchorIDsByCenterChainNodeID: anchorIDs, Counts: counts,
	}, nil
}

func validateReferences(ctx context.Context, tx Transaction, publication Publication) error {
...
func referencedIDs(publication Publication) ([]string, []string) {
