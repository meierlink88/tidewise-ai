...
func TestPostgresRelationshipBatchLocksPersistedEndpointsAndCommits(t *testing.T) {
	...
	mock.ExpectBegin()
	mock.ExpectQuery("SELECT entity_key, entity_type, status.*FOR SHARE").WithArgs("chain_node:test").WillReturnRows(sqlmock.NewRows([]string{"entity_key", "entity_type", "status"}).AddRow("chain_node:test", model.EntityTypeChainNode, model.StatusActive))
	mock.ExpectQuery("SELECT entity_key, entity_type, status.*FOR SHARE").WithArgs("sector:test").WillReturnRows(sqlmock.NewRows([]string{"entity_key", "entity_type", "status"}).AddRow("sector:test", model.EntityTypeSector, model.StatusActive))
...
func TestPostgresRelationshipBatchRollsBackMissingEndpointAndIdentityConflict(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "identity conflict", secondRows: sqlmock.NewRows([]string{"entity_key", "entity_type", "status"}).AddRow("sector:test", model.EntityTypeSector, model.StatusActive), action: "identity_conflict"},
	} {
...
func TestMemoryRelationshipBatchRejectsPolicyAndIdentityAtomically(t *testing.T) {
	...
	for _, entity := range []Entity{
		{Key: "chain_node:test", EntityType: model.EntityTypeChainNode, LayerCode: "industry_chain", Name: "节点", CanonicalName: "节点", Aliases: []string{"Test Node"}, Status: model.StatusActive},
		{Key: "sector:test", EntityType: model.EntityTypeSector, LayerCode: "sector", Name: "板块", CanonicalName: "板块", Aliases: []string{"Test Sector"}, Status: model.StatusActive},
		{Key: "sector:other", EntityType: model.EntityTypeSector, LayerCode: "sector", Name: "其他", CanonicalName: "其他", Aliases: []string{"Other Sector"}, Status: model.StatusActive},
	} {
...
func reviewedSectorMapping(key, from, to string) Relationship {
	return Relationship{Key: key, From: from, To: to, RelationType: "mapped_to_sector", EvidenceNote: "composite curation", SourceName: "Tidewise review", SourceURL: "https://example.com/review", VerifiedAt: time.Date(2026, 7, 13, 0, 0, 0, 0, time.UTC), Status: model.StatusActive}
}
