...
func TestMemoryRepositoryRejectsIndustryChainIdentityMutationAtomically(t *testing.T) {
	...
	changed := batch
	changed.Memberships = append([]model.IndustryChainMembership(nil), batch.Memberships...)
	changed.Memberships[0].ChainNodeEntityID = "node-c"
	changed.TopologyEdges = append([]model.IndustryChainTopologyEdge(nil), batch.TopologyEdges...)
	changed.TopologyEdges[0].FromChainNodeEntityID = "node-c"
	changed.PhysicalConstraints = append([]model.IndustryChainPhysicalConstraint(nil), batch.PhysicalConstraints...)
...
func TestMemoryRepositoryConstraintOnlyUsesPersistedSubjectsAndApprovalGate(t *testing.T) {
	...
	batch := IndustryChainBatch{PhysicalConstraints: []model.IndustryChainPhysicalConstraint{constraint}, ApprovalGate: model.IndustryChainApprovalGate{HumanApprovedConstraintIDs: map[string]struct{}{"constraint": {}}}}
	if _, err := repo.UpsertIndustryChainBatch(context.Background(), batch); err != nil {
	...
	batch.ApprovalGate = model.IndustryChainApprovalGate{}
...
func TestPostgresRepositoryConstraintOnlyLocksSubjectsInStableOrder(t *testing.T) {
	...
	batch := IndustryChainBatch{PhysicalConstraints: []model.IndustryChainPhysicalConstraint{first, second}}
	if _, err := NewPostgresRepository(db).UpsertIndustryChainBatch(context.Background(), batch); err != nil {
		t.Fatalf("constraint-only batch: %v", err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
...
func TestPostgresRepositoryConstraintOnlyLocksPersistedTopologySubject(t *testing.T) {
	...
	if _, err := NewPostgresRepository(db).UpsertIndustryChainBatch(context.Background(), IndustryChainBatch{PhysicalConstraints: []model.IndustryChainPhysicalConstraint{constraint}}); err != nil {
		t.Fatalf("constraint-only batch: %v", err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatal(err)
...
func TestIndustryChainRepositoriesShareAssociationAndApprovalValidation(t *testing.T) {
	...
	aiApproved.ApprovalGate = model.IndustryChainApprovalGate{HumanApprovedConstraintIDs: map[string]struct{}{"constraint": {}}}
...
func validIndustryChainBatch() IndustryChainBatch {
	...
	return IndustryChainBatch{
		Profiles: []model.IndustryChainProfile{{EntityID: "chain", ChainCode: "test", Definition: "test", ScopeType: model.IndustryChainScopeGlobal, Version: 1, ReviewStatus: model.ReviewStatusApproved, SourceName: "review", SourceURL: "https://example.com", VerifiedAt: verifiedAt}},
		Memberships: []model.IndustryChainMembership{
			{ID: "membership-a", IndustryChainEntityID: "chain", ChainNodeEntityID: "node-a", StageCode: model.IndustryChainStageUpstream, RoleCode: model.IndustryChainRoleComponent, SourceName: "review", SourceURL: "https://example.com", VerifiedAt: verifiedAt, Status: model.StatusActive},
			{ID: "membership-b", IndustryChainEntityID: "chain", ChainNodeEntityID: "node-b", StageCode: model.IndustryChainStageDownstream, RoleCode: model.IndustryChainRoleProduct, SourceName: "review", SourceURL: "https://example.com", VerifiedAt: verifiedAt, Status: model.StatusActive},
		},
		TopologyEdges:       []model.IndustryChainTopologyEdge{{ID: "edge", IndustryChainEntityID: "chain", FromChainNodeEntityID: "node-a", ToChainNodeEntityID: "node-b", RelationType: model.IndustryChainRelationSuppliesTo, SourceName: "review", SourceURL: "https://example.com", VerifiedAt: verifiedAt, Status: model.StatusActive}},
		PhysicalConstraints: []model.IndustryChainPhysicalConstraint{{ID: "constraint", IndustryChainEntityID: "chain", ChainNodeEntityID: "node-a", ConstraintType: model.PhysicalConstraintPowerCapacity, Mechanism: "power", SourceName: "review", SourceURL: "https://example.com", VerifiedAt: verifiedAt, ReviewStatus: model.ReviewStatusApproved, Status: model.StatusActive}},
	}
}
