...
const frozenChainNodeRelationSourceReferencePath = "openspec/changes/rebuild-foundation-graph-and-enrich-chain-data/reviews/chain-node-relations-r0/approved-candidate-manifest.json"
const frozenChainNodeRelationDataRelativeDir = "analyse-data-service/backend/data/entity_foundation/relationships/reviewed_chain_node_relations"
...
const frozenAdditiveChainNodeRelationManifestRelativePath = frozenChainNodeRelationDataRelativeDir + "/additive-final-candidate-manifest.json"
...
type frozenAdditiveChainNodeRelationManifest struct {
	...
	ExistingByRelationType         map[model.ChainNodeRelationType]int `json:"existing_by_relation_type"`
	NewRelationCount               int                                 `json:"new_relation_count"`
	NewByRelationType              map[model.ChainNodeRelationType]int `json:"new_by_relation_type"`
	NewByEvidenceTier              map[string]int                      `json:"new_by_evidence_tier"`
	TotalRelationCount             int                                 `json:"total_relation_count"`
	TotalByRelationType            map[model.ChainNodeRelationType]int `json:"total_by_relation_type"`
...
func LoadFrozenChainNodeRelationManifest(path string) (ChainNodeRelationManifest, error) {
	...
	manifest := ChainNodeRelationManifest{Relations: make([]model.ChainNodeRelation, 0, len(frozen.Relations))}
...
func LoadFrozenAdditiveChainNodeRelationManifest(path string) (ChainNodeRelationManifest, error) {
	...
	additive := ChainNodeRelationManifest{Relations: make([]model.ChainNodeRelation, 0, len(frozen.NewRelations))}
	...
	combined := ChainNodeRelationManifest{Relations: make([]model.ChainNodeRelation, 0, len(accepted.Relations)+len(additive.Relations))}
	...
	if err := model.ValidateChainNodeRelationBatch(combined.Relations); err != nil {
		return ChainNodeRelationManifest{}, err
	}
	return combined, nil
}
...
func validateFrozenAdditiveChainNodeRelationMetadata(frozen frozenAdditiveChainNodeRelationManifest, accepted, additive, combined ChainNodeRelationManifest) error {
	wantAccepted := map[model.ChainNodeRelationType]int{
		model.ChainNodeRelationSubcategoryOf: 95,
		model.ChainNodeRelationComponentOf:   1,
		model.ChainNodeRelationInputTo:       3,
		model.ChainNodeRelationDependsOn:     1,
	}
	wantAdditive := map[model.ChainNodeRelationType]int{
		model.ChainNodeRelationSubcategoryOf: 13,
		model.ChainNodeRelationComponentOf:   2,
		model.ChainNodeRelationInputTo:       90,
		model.ChainNodeRelationDependsOn:     7,
	}
	wantCombined := map[model.ChainNodeRelationType]int{
	...
	if reference.Path != frozenChainNodeRelationSourceReferencePath || reference.FileSHA256 != frozenChainNodeRelationManifestFileSHA256 || reference.ContentSHA256 != frozenChainNodeRelationContentSHA256 || reference.ManifestSHA256 != frozenChainNodeRelationManifestSHA256 || reference.PreservedContentDiff != 0 {
...
func equalFrozenChainNodeRelationCounts(got, want map[model.ChainNodeRelationType]int) bool {
	if len(got) != len(want) {
		return false
	}
...
func validateFrozenChainNodeRelationIDs(relations []model.ChainNodeRelation) error {
...
func validateFrozenChainNodeRelationMetadata(frozen frozenChainNodeRelationManifest, manifest ChainNodeRelationManifest) error {
	...
	counts := map[model.ChainNodeRelationType]int{}
	for index, relation := range manifest.Relations {
		...
		if relation.ID != bizidentity.NormalizeUUID("chain_node_relation", identity) {
			return fmt.Errorf("relation %q deterministic id mismatch", relation.ID)
		}
		if relation.Status != model.StatusActive || relation.VerifiedAt.IsZero() {
	...
	want := map[model.ChainNodeRelationType]int{
		model.ChainNodeRelationSubcategoryOf: 95,
		model.ChainNodeRelationComponentOf:   1,
		model.ChainNodeRelationInputTo:       3,
		model.ChainNodeRelationDependsOn:     1,
	...
	return model.ValidateChainNodeRelationBatch(manifest.Relations)
}
...
func loadFrozenChainNodeEndpointBaseline() (map[string]struct{}, error) {
	...
	for _, node := range baseline.Nodes {
		if node.EntityID == "" || node.EntityKey == "" || node.Name == "" || node.CanonicalName == "" || node.Status != model.StatusActive {
...
func validateFrozenChainNodeRelationEndpoints(relations []model.ChainNodeRelation, endpoints map[string]struct{}) error {
...
func frozenChainNodeRelationArtifactPath(name string) (string, error) {
	...
	return filepath.Join(repositoryRoot, frozenChainNodeRelationDataRelativeDir, name), nil
}
