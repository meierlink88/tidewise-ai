...
func TestFirstBatchExternalIdentifierValidation(t *testing.T) {
	...
	for _, candidate := range []model.EntityExternalIdentifier{
...
func TestMemoryRepositoryUpsertsExternalIdentifiersIdempotentlyAndRejectsRebinding(t *testing.T) {
	...
	for _, entity := range []Entity{
		{Key: "chain_node:3d_printing", EntityType: model.EntityTypeChainNode, LayerCode: "chain_node", Name: "3D打印", CanonicalName: "3D打印", Status: model.StatusActive},
		{Key: "chain_node:additive_manufacturing", EntityType: model.EntityTypeChainNode, LayerCode: "chain_node", Name: "增材制造", CanonicalName: "增材制造", Status: model.StatusActive},
	} {
...
func TestPostgresExternalIdentifierUpsertUpdatesSameEntityAfterLock(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT id, entity_id, external_name, status FROM entity_external_identifiers")).
		WithArgs(identifier.SourceSystem, identifier.SourceTaxonomyType, identifier.ExternalCode).
		WillReturnRows(sqlmock.NewRows([]string{"id", "entity_id", "external_name", "status"}).AddRow(identifier.ID, identifier.EntityID, "旧名称", model.StatusInactive))
