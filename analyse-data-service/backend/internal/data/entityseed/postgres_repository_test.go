...
func TestPostgresEntityWriteNormalizesNilAliases(t *testing.T) {
	entity := Entity{
		Key:           "sector:test",
		EntityType:    model.EntityTypeSector,
		LayerCode:     "sector",
		Name:          "测试板块",
		CanonicalName: "测试板块",
	}
...
func TestProfileTableName(t *testing.T) {
	cases := map[model.EntityType]string{
...
func TestIndustryChainAndChainNodeProfileUpsertFields(t *testing.T) {
	chainSQL, _, err := buildProfileUpsert("industry_chain:test", model.EntityTypeIndustryChain, []byte(`{"chain_code":"test","definition":"test","boundary_note":"","scope_type":"global","version":1,"review_status":"approved","source_name":"review","source_url":"https://example.com","verified_at":"2026-07-12T00:00:00Z"}`))
	if err != nil {
		t.Fatal(err)
	}
	...
	nodeSQL, _, err := buildProfileUpsert("chain_node:test", model.EntityTypeChainNode, []byte(`{"chain_position":"midstream","node_category":"component","definition":"test","unit_of_analysis":"component","granularity_note":"test"}`))
	if err != nil {
		t.Fatal(err)
	}
...
func TestProfileUpsertSQLIncludesSectorSnapshotFields(t *testing.T) {
	statement, _, err := buildProfileUpsert("sector:ai", model.EntityTypeSector, []byte(`{
		"sector_system": "ths",
...
func TestProfileUpsertSQLIncludesMarketSectorFields(t *testing.T) {
	statement, _, err := buildProfileUpsert("sector:theme_ai", model.EntityTypeSector, []byte(`{
