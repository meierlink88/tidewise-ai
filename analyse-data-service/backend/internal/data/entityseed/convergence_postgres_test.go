...
func TestPostgresIndexTargetRedirectsPolicyCompatibleGenericEdge(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT e.id, fn.entity_key, fn.entity_type, tn.entity_key, tn.entity_type,")).
		...
		WillReturnRows(sqlmock.NewRows([]string{"id", "from_key", "from_type", "to_key", "to_type", "relation_type", "evidence_note", "source_name", "source_url", "verified_at", "status"}).
			AddRow("11111111-1111-1111-1111-111111111111", "market:test", model.EntityTypeMarket, "sector:legacy", model.EntityTypeSector, "tracks_index", "reviewed identity correction", "review", "https://example.com/review", verifiedAt, model.StatusActive))
...
func TestPostgresIndexTargetBlocksSectorOnlyForeignKey(t *testing.T) {
	...
	manifest := SectorConvergenceManifest{Convergences: []SectorConvergence{{LegacyEntityKey: "sector:legacy", TargetEntityKey: "index:csi300", TargetEntityType: model.EntityTypeIndex}}}
...
func TestPostgresConvergenceAuditSnapshotsTargetTypeAndReason(t *testing.T) {
	item := SectorConvergence{LegacyTaxonomy: "index", Action: SectorConvergenceReplaceWithExistingIndex, TargetEntityType: model.EntityTypeIndex, Reason: "reviewed equivalent index identity"}
	...
	if got := args[3]; got != model.EntityTypeIndex {
...
func TestPostgresCorrectionRedirectsRecordedSectorEdgeToIndex(t *testing.T) {
	...
	mock.ExpectQuery("SELECT target_entity_id, target_entity_type FROM entity_convergences").WithArgs(previousID).WillReturnRows(sqlmock.NewRows([]string{"target_entity_id", "target_entity_type"}).AddRow(oldID, model.EntityTypeSector))
	...
	mock.ExpectQuery("SELECT e.id, fn.id, fn.entity_key, fn.entity_type, tn.id, tn.entity_key, tn.entity_type").WithArgs(edgeID).WillReturnRows(correctionEdgeRows().AddRow(edgeID, entitySeedUUID("market:test"), "market:test", model.EntityTypeMarket, oldID, "sector:old", model.EntityTypeSector, "tracks_index", "reviewed", "review", "https://example.com/review", time.Date(2026, 7, 12, 0, 0, 0, 0, time.UTC), model.StatusActive))
	mock.ExpectExec(regexp.QuoteMeta("UPDATE entity_edges SET to_entity_id=$1,status='active',updated_at=NOW() WHERE id=$2 AND to_entity_id=$3 AND status=$4")).WithArgs(newID, edgeID, oldID, model.StatusActive).WillReturnResult(sqlmock.NewResult(0, 1))
	...
	counts, err := applyRecordedCorrectionMoves(context.Background(), tx, previousID, currentID, model.EntityTypeIndex, newID, "index:new")
	if err != nil {
		t.Fatal(err)
	}
...
func TestPostgresCorrectionReactivatesRecordedNoTargetEdgeToIndex(t *testing.T) {
	...
	mock.ExpectQuery("SELECT e.id, fn.id, fn.entity_key, fn.entity_type, tn.id, tn.entity_key, tn.entity_type").WithArgs(edgeID).WillReturnRows(correctionEdgeRows().AddRow(edgeID, entitySeedUUID("market:test"), "market:test", model.EntityTypeMarket, legacyID, "sector:legacy", model.EntityTypeSector, "tracks_index", "reviewed", "review", "https://example.com/review", time.Date(2026, 7, 12, 0, 0, 0, 0, time.UTC), model.StatusInactive))
	mock.ExpectExec(regexp.QuoteMeta("UPDATE entity_edges SET to_entity_id=$1,status='active',updated_at=NOW() WHERE id=$2 AND to_entity_id=$3 AND status=$4")).WithArgs(newID, edgeID, legacyID, model.StatusInactive).WillReturnResult(sqlmock.NewResult(0, 1))
...
func TestPostgresCorrectionDeactivatesRecordedTargetEdgeForNoTarget(t *testing.T) {
	...
	mock.ExpectQuery("SELECT target_entity_id, target_entity_type FROM entity_convergences").WithArgs(previousID).WillReturnRows(sqlmock.NewRows([]string{"target_entity_id", "target_entity_type"}).AddRow(oldID, model.EntityTypeSector))
	mock.ExpectQuery("SELECT reference_table,reference_column,reference_row_id,from_entity_id,to_entity_id").WithArgs(previousID).WillReturnRows(sqlmock.NewRows([]string{"reference_table", "reference_column", "reference_row_id", "from_entity_id", "to_entity_id"}).AddRow("entity_edges", "to_entity_id", edgeID, entitySeedUUID("sector:legacy"), oldID))
	mock.ExpectQuery("SELECT e.id, fn.id, fn.entity_key, fn.entity_type, tn.id, tn.entity_key, tn.entity_type").WithArgs(edgeID).WillReturnRows(correctionEdgeRows().AddRow(edgeID, entitySeedUUID("market:test"), "market:test", model.EntityTypeMarket, oldID, "sector:old", model.EntityTypeSector, "covers_sector", "reviewed", "review", "https://example.com/review", time.Date(2026, 7, 12, 0, 0, 0, 0, time.UTC), model.StatusActive))
	mock.ExpectExec(regexp.QuoteMeta("UPDATE entity_edges SET status='inactive',updated_at=NOW() WHERE id=$1 AND status=$2")).WithArgs(edgeID, model.StatusActive).WillReturnResult(sqlmock.NewResult(0, 1))
...
func TestPostgresCorrectionIncompatiblePolicyRollsBack(t *testing.T) {
	...
	mock.ExpectQuery("SELECT reference_table,reference_column,reference_row_id,from_entity_id,to_entity_id").WithArgs(previousID).WillReturnRows(sqlmock.NewRows([]string{"reference_table", "reference_column", "reference_row_id", "from_entity_id", "to_entity_id"}).AddRow("entity_edges", "to_entity_id", edgeID, entitySeedUUID("sector:legacy"), oldID))
	mock.ExpectQuery("SELECT e.id, fn.id, fn.entity_key, fn.entity_type, tn.id, tn.entity_key, tn.entity_type").WithArgs(edgeID).WillReturnRows(correctionEdgeRows().AddRow(edgeID, entitySeedUUID("market:test"), "market:test", model.EntityTypeMarket, oldID, "sector:old", model.EntityTypeSector, "covers_sector", "reviewed", "review", "https://example.com/review", time.Date(2026, 7, 12, 0, 0, 0, 0, time.UTC), model.StatusActive))
	if _, err := applyRecordedCorrectionMoves(context.Background(), tx, previousID, currentID, model.EntityTypeIndex, newID, "index:new"); err == nil || !strings.Contains(err.Error(), "incompatible") {
		t.Fatalf("error = %v", err)
	}
	mock.ExpectRollback()
	if err := tx.Rollback(); err != nil {
...
func TestPostgresCorrectionDriftRollsBack(t *testing.T) {
	...
	mock.ExpectQuery("SELECT reference_table,reference_column,reference_row_id,from_entity_id,to_entity_id").WithArgs(previousID).WillReturnRows(sqlmock.NewRows([]string{"reference_table", "reference_column", "reference_row_id", "from_entity_id", "to_entity_id"}).AddRow("entity_edges", "to_entity_id", edgeID, entitySeedUUID("sector:legacy"), oldID))
	mock.ExpectQuery("SELECT e.id, fn.id, fn.entity_key, fn.entity_type, tn.id, tn.entity_key, tn.entity_type").WithArgs(edgeID).WillReturnRows(correctionEdgeRows().AddRow(edgeID, entitySeedUUID("market:test"), "market:test", model.EntityTypeMarket, oldID, "sector:old", model.EntityTypeSector, "covers_sector", "reviewed", "review", "https://example.com/review", time.Date(2026, 7, 12, 0, 0, 0, 0, time.UTC), model.StatusInactive))
	if _, err := applyRecordedCorrectionMoves(context.Background(), tx, previousID, currentID, model.EntityTypeSector, newID, "sector:new"); err == nil || !strings.Contains(err.Error(), "drifted") {
		t.Fatalf("error = %v", err)
	}
	mock.ExpectRollback()
	if err := tx.Rollback(); err != nil {
