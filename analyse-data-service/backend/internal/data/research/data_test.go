...
INSERT INTO variable_definitions (
	...
	if len(firstPage.Dictionaries.EntityTypeDefinitions) == 0 {
		t.Fatal("Analysis Context has no referenced Entity Type Definition")
	}
	referencedType := firstPage.Dictionaries.EntityTypeDefinitions[0].TypeKey
	for version, createdAt := range map[int]time.Time{2: now.Add(-30 * time.Minute), 3: now.Add(time.Hour)} {
		if _, err := db.ExecContext(ctx, `INSERT INTO entity_type_definitions (
    type_key, version, signal_subject_allowed, direct_target_mode, status, created_at,
    allowed_event_roles, name_zh, name_en, business_definition, inclusion_criteria,
    exclusion_criteria, event_link_allowed
)
SELECT type_key, $2, signal_subject_allowed, direct_target_mode, status, $3,
       allowed_event_roles, name_zh, name_en, business_definition, inclusion_criteria,
       exclusion_criteria, event_link_allowed
FROM entity_type_definitions WHERE type_key = $1 AND version = 1`, referencedType, version, createdAt); err != nil {
			t.Fatalf("seed Entity Type version %d: %v", version, err)
		}
	}
	request.Cursor = ""
	versionedPage, err := contextService.List(ctx, request)
	if err != nil {
		t.Fatalf("list Analysis Context with versioned Entity Type Definitions: %v", err)
	}
	versions := make(map[int]struct{})
	for _, definition := range versionedPage.Dictionaries.EntityTypeDefinitions {
		if definition.TypeKey == referencedType {
			versions[definition.Version] = struct{}{}
		}
	}
	if _, ok := versions[1]; !ok {
		t.Fatalf("Entity Type versions = %#v, want existing version 1", versions)
	}
	if _, ok := versions[2]; !ok {
		t.Fatalf("Entity Type versions = %#v, want as-of version 2", versions)
	}
	if _, ok := versions[3]; ok {
		t.Fatalf("future Entity Type version leaked into past Analysis Context: %#v", versions)
	}
	...
	var preparedSnapshot researchbiz.SnapshotAggregate
	...
	preparedTheme, err := readService.GetTheme(ctx, preparedPublished.ThemeID, researchbiz.ResearchDetailRequest{WindowHours: 24})
	if err != nil {
...
func seedPreparedSnapshotReferences(
	...
	ctx context.Context,
	db *sql.DB,
	aggregate researchbiz.SnapshotAggregate,
) {
	t.Helper()
    ...
    evidence_relation, supports_fields, contract_version
				...
				evidenceID, event.EventID, researchEvidenceHash("Prepared UAT Evidence")); err != nil {
...
func assertPreparedThemeReadback(
	t *testing.T,
	actual researchbiz.ResearchThemeDetail,
	expected researchbiz.SnapshotAggregate,
) {
	t.Helper()
	theme := actual.Theme
	want := expected.Theme
	if actual.ThemeKey != want.ThemeKey || actual.PublicationMode != researchbiz.SnapshotPublicationMode ||
...
func assertPreparedTreeReadback(
	t *testing.T,
	actual researchbiz.ResearchReasoningTreeDetail,
	expected researchbiz.SnapshotReasoningTree,
) {
	t.Helper()
	tree := actual.ReasoningTree
	if actual.ThemeKey != "theme:at-01" || actual.PublicationMode != researchbiz.SnapshotPublicationMode ||
