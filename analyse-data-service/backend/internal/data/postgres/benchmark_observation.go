...
import (
	...
	domain "github.com/meierlink88/tidewise-ai/analyse-data-service/backend/internal/biz/model"
)
...
func (r *InMemoryRepository) UpsertBenchmarkObservation(_ context.Context, observation domain.BenchmarkObservation) (BenchmarkObservationWriteResult, error) {
	...
	entityType, ok := r.entityTypes[observation.BenchmarkEntityID]
	if !ok {
	...
	if entityType != domain.EntityTypeBenchmark {
		return BenchmarkObservationWriteResult{}, fmt.Errorf("entity %q type %q is not benchmark", observation.BenchmarkEntityID, entityType)
	}
...
func (r repository) UpsertBenchmarkObservation(ctx context.Context, observation domain.BenchmarkObservation) (BenchmarkObservationWriteResult, error) {
	if err := observation.Validate(); err != nil {
		return BenchmarkObservationWriteResult{}, err
	}
...
FROM upsert
`, bizidentity.NormalizeUUID(observation.ID), bizidentity.NormalizeUUID(observation.BenchmarkEntityID), observation.ObservedAt, observation.Value, observation.Unit, observation.SourceName,
...
func (r repository) ListBenchmarkObservations(ctx context.Context, filter BenchmarkObservationFilter) ([]domain.BenchmarkObservation, error) {
...
func (r repository) ensureBenchmarkEntity(ctx context.Context, entityID string) error {
	var entityType domain.EntityType
	if err := r.db.QueryRowContext(ctx, `
SELECT entity_type
FROM entity_nodes
WHERE id = $1
`, bizidentity.NormalizeUUID(entityID)).Scan(&entityType); err != nil {
