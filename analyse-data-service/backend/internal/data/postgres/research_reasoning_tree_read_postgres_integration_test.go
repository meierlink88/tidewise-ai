...
import (
	...
	anchorapp "github.com/meierlink88/tidewise-ai/analyse-data-service/backend/internal/biz/researchanchorimport"
...
func TestResearchReasoningTreeReadPostgresIntegration(t *testing.T) {
	...
	repository := postgres.NewResearchRepository(db)
	homeFilter := postgres.ResearchThemeListFilter{
	...
	imported, err := anchorapp.NewService(postgres.NewResearchAnchorImportStore(db)).Import(
