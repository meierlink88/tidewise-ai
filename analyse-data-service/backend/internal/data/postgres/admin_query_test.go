...
func TestInMemoryRepositoryListsRawDocumentsWithPaginationAndTitleSearch(t *testing.T) {
	...
	for _, doc := range []model.RawDocument{
...
func TestInMemoryRepositoryListsEventsWithFilters(t *testing.T) {
	...
	if err := repo.SeedEvent(context.Background(), model.Event{
		ID:          "event-1",
		Title:       "美联储维持利率不变",
		Summary:     "FOMC 会议维持联邦基金利率目标区间不变。",
		EventTime:   &eventTime,
		FirstSeenAt: eventTime.Add(10 * time.Minute),
		EventStatus: model.EventStatusConfirmed,
		FactStatus:  model.FactStatusVerified,
	...
	if err := repo.SeedEvent(context.Background(), model.Event{
		...
		EventStatus: model.EventStatusCandidate,
		FactStatus:  model.FactStatusUnverified,
...
func validRawDocumentWithTitle(id string, title string, collectedAt time.Time) model.RawDocument {
...
func validRawDocument(id string) model.RawDocument {
	return model.RawDocument{
		...
		ContentHash:   id + "-hash",
		IngestStatus:  model.IngestStatusCollected,
	}
}

func rawDocumentIDs(items []model.RawDocument) []string {
