...
func run() int {
	...
	if err != nil {
		logger.Error("load Data config", slog.String("error", err.Error()))
		return 1
	}
	app, cleanup, err := buildApp(config, logger)
	if err != nil {
		logger.Error("compose Data service", slog.String("error", err.Error()))
		return 1
	}
	...
	if err := runApplication(app, cleanup); err != nil {
		logger.Error("Data service failed",
			slog.String("service", conf.ServiceName),
