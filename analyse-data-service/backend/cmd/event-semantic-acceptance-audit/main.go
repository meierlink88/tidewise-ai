...
var categories = []string{
	"correct_reject", "abox_missing", "tbox_out_of_scope", "mention_extraction_miss",
	"retrieval_miss", "selector_false_reject", "review_reject", "model_contract_failure",
}

var classificationPrecedence = []string{
	"model_contract_failure", "mention_extraction_miss", "tbox_out_of_scope", "retrieval_miss",
...
func loadEntities(ctx context.Context, database *sql.DB) ([]entityIdentity, error) {
	rows, err := database.QueryContext(ctx, `
		SELECT entity.id, entity.entity_type, entity.name, entity.canonical_name,
		       ...
		       COALESCE(definition.event_link_allowed, false)
		FROM entity_nodes entity
		LEFT JOIN LATERAL (
		    SELECT event_link_allowed FROM entity_type_definitions
		    WHERE type_key = entity.entity_type AND status = 'active'
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.EntityType, &item.Name, &item.CanonicalName, &aliasesJSON, &item.Status, &item.EventLinkAllowed); err != nil {
			return nil, err
		}
...
func classify(run acceptanceRun, text eventText, referenceMentions []string, entities []entityIdentity) (string, string) {
	...
	for _, entity := range formalInText {
		if entity.Status == "active" && entity.EventLinkAllowed && !identityExtracted(entity, mentions) {
			return "mention_extraction_miss", "formal in-scope identity was not extracted: " + entity.CanonicalName
		}
	}
	for _, entity := range formalInText {
		if entity.Status != "active" || !entity.EventLinkAllowed {
			return "tbox_out_of_scope", "formal identity is not active/event-link-allowed: " + entity.CanonicalName
		}
	}
	...
	for key, mention := range mentions {
		for _, entity := range exactIdentities(mention, entities) {
			if entity.Status != "active" || !entity.EventLinkAllowed {
				return "tbox_out_of_scope", "extracted identity is not active/event-link-allowed: " + entity.CanonicalName
			}
