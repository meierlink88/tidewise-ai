# Tavily Capital-Market Search Specification

## Purpose

定义 AI 采集器如何通过项目内 Tavily HTTP adapter 执行面向资本市场资讯的确定性搜索，包括受控证据选择的默认 topic、结果预算、query-related 内容边界、单次调用语义与隔离真实服务的契约测试。
