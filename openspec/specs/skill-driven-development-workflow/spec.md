...
系统 MUST 将 `AGENTS.md` 作为最高级硬门与路由入口，将 `openspec/config.yaml` 限定为稳定项目背景、语言和 artifact 写作约束，并分别以 `.agents/openspec-workflow.md`、`.agents/git-workflow.md`、`.agents/testing-tdd.md` 作为 OpenSpec/审批、Git/交付、TDD/验证的唯一完整详述来源。主 workflow spec 只描述长期可验证行为。

#### Scenario: 查找专责规则

- **WHEN** Agent 需要生命周期、Git 或测试操作细节
- **THEN** Agent 从根入口路由到对应 `.agents` 专责文件，不要求其他文件复制完整流程

#### Scenario: 配置生成 artifacts

- **WHEN** OpenSpec CLI 生成 artifact instructions
- **THEN** config 提供稳定上下文和写作约束，不承载操作流程、历史迁移或一次性验收指标
...
系统 MUST 在分层规则中继续明确 OpenSpec 唯一生命周期、人工 Review、有状态操作审批、Sync/Archive/Deliver 顺序、Desktop-managed 入口、branch/worktree 隔离、两类 cleanup、PostgreSQL 事实源、Neo4j 可重建投影和通用安全边界。
...
系统 MUST 统一 `gate`、`package`、`checkpoint` 和 `commit`：gate 是人工 Review/Authorization/Acceptance 边界，package 是同一风险边界内连续完成的内聚交付单元，checkpoint 是可审阅证据点，commit 是 scoped Git 状态快照。普通 task、测试、修复、diff、commit 和 push 不得单独制造人工 gate。
