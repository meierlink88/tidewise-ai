...
定义市场板块实体、分类法、候选准入、稳定标识、客观关系和 canonical active 集合的当前系统事实。
...
#### Scenario: 使用独立评分锚点
- **WHEN** 开发者为候选六个评分维度打分
- **THEN** 每个维度必须独立使用 design 定义的 0/3/5 证据锚点，1、2、4 只能表示相邻锚点之间的中间状态，不得使用其他维度的证据抬高本项得分

#### Scenario: 评分事件可解释性和传导独立性
- **WHEN** 开发者评估事件可解释性或传导独立性
- **THEN** 事件可解释性必须依据可列举事件类型、影响通道和证据链评分，传导独立性必须依据相对相邻候选的暴露边界、传导节点和差异响应路径评分

#### Scenario: 评分行情敏感度和数据完整性
- **WHEN** 开发者评估行情敏感度或数据完整性
- **THEN** 行情敏感度必须依据连续行情中的事件窗口响应或统计证据评分，数据完整性必须依据来源定义、稳定 mapping、代码、方法/成分说明、行情连续性和更新时间的可追溯程度评分

#### Scenario: 评分长期稳定性和市场代表性
- **WHEN** 开发者评估长期稳定性或市场代表性
- **THEN** 长期稳定性必须依据名称、范围、方法定义的跨周期持续性和变更可追踪性评分，市场代表性必须依据目标市场或传导簇覆盖、成分规模、权威分类或多来源采用证据评分
