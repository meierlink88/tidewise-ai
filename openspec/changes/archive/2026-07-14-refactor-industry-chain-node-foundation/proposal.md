...
前序 `add-industry-chain-node-foundation` 将产业链表达拆成 sector、industry_chain、membership 与 topology 多套并行结构，造成身份重复、层级固化、关系语义重叠，也难以支撑事件实体链接与可解释推理。本 change 先建立统一最小结构和完整只读 preflight，再在独立授权下以版本化 migration 清除 PostgreSQL 旧产业模型及其遗留事实。结构实现 checkpoint `0f20171` 已通过人工复验；第一批 chain_node 名称范围现已批准，UUID/key、definition/boundary 内容与可执行 seed 仍须按独立门禁继续 Review。
...
- 不恢复 `sector_source_mappings`，不建设语义含混的 `chain_node_source_mappings`，也不把 provider/code 写入 `chain_node_profiles`。新增通用 `entity_external_identifiers`，用于规范保存内部实体与外部系统标识；本 change 首批只允许写入已审阅 chain_node 的东方财富/同花顺代码，不顺带迁移旧 sector mapping 或其他实体。
...
- `entity-external-identifiers`: 定义通用实体外部标识表、外部 identity 唯一性、首批 chain_node 映射边界与分层写入门禁。

### Modified Capabilities

- `event-knowledge-schema`: 将产业实体类型收敛为 chain_node，并增加 theme 实体类型，避免事件实体链接继续产生 sector 或 industry_chain 容器身份。
- `entity-foundation-seeds`: 切断 sector 来源映射、industry_chain 容器与 memberships 的默认生产初始化入口；将已批准的 842 个 chain_node 名称范围、aliases、definition/boundary、全新身份、dry-run/report 和 1,156 条外部标识纳入后续实现门禁。
...
- structure implementation checkpoint 已影响 `backend/migrations/`、`backend/internal/domain/`、实体 seed/service/repository/CLI 边界及相关测试；本次 data contract review 只修改 OpenSpec artifacts，不修改 migration、源码或 seed 数据。关系与 physical constraint 的新实现仍留在 Phase B。
