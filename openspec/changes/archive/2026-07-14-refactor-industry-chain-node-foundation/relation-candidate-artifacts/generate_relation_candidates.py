...
RULE_VERSION = "phase-b-semantic-v2-second-review"
...
def candidate(kind,a,b,mechanism,rule,confidence="high"):
    return {"disposition":"ai_approved_candidate_draft","relation_type":kind,"from":endpoint(a),"to":endpoint(b),"direction":f'{a["name"]} -> {b["name"]}',"mechanism":mechanism,"condition_note":"仅限已批准 definition/boundary 所表达的稳定语义范围","derivation_evidence":{"source":"approved internal source artifact","artifact_path":str(INPUT.relative_to(ROOT)),"input_sha256":sha(INPUT),"rule":rule,"review_record":["ffb243e Sol Medium first pass","main Serenity independent second pass"]},"provenance":f"{RULE_VERSION}:{rule}","confidence":confidence,"counterexample":"若子项名称只是市场标签、并列复合词或边界未覆盖全部实例，则关系不成立","uncertainty":"双遍 AI Review 已通过；证据契约 amendment 尚待用户确认，当前不是可执行 manifest"}
...
def main():
    ...
    for a,b,reason in [
      ("稀土产业","稀土永磁","from endpoint 是产业范围，不是可被直接消耗的具体材料输出；未来应改用具体稀土材料节点重新提议"),
      ("铜产业","铜缆高速连接","from endpoint 是产业范围，不是可被直接消耗的具体铜材料输出；未来应改用具体材料节点重新提议")]:
        if a in nodes and b in nodes:
            rejected.append({"disposition":"rejected","relation_type":"input_to","from":endpoint(nodes[a]),"to":endpoint(nodes[b]),"direction":f"{a} -> {b}","reason":reason,"review_record":"main Serenity independent second pass","future_action":"replace endpoint and submit a new evidence-backed candidate"})
    physical=[]
    for name,ctype,desc,missing in [
      ("半导体","process_yield","制造良率可能构成可量化硬工艺约束","subject 过宽；先收窄到具体工艺节点或关系，再补工艺节点良率、设备/材料敏感性与产能损失来源"),
      ...
      ("电池","material_purity","关键材料纯度可能约束产品性能与良率","subject 过宽；先收窄到具体材料节点或关系，再补材料规格、失效机理与资格认证来源")]:
    ...
    reviewable=sorted(reviewable,key=lambda x:(x["relation_type"],x["direction"])); blocked_items=sorted(blocked_items,key=lambda x:(x["relation_type"],x["direction"])); rejected=sorted(rejected,key=lambda x:x.get("direction",str(x.get("candidate"))))
    out={"artifact_type":"phase_b_relation_candidate_review","rule_version":RULE_VERSION,"ready_for_write":False,"input":{"path":str(INPUT.relative_to(ROOT)),"sha256":sha(INPUT),"node_count":len(nodes)},"counts":{"approved_semantic":len(reviewable),"blocked_needs_evidence":len(blocked_items),"physical_constraint_blocked":len(physical),"rejected":len(rejected),"approved_by_relation_type":{},"blocked_by_relation_type":{}},"approved_semantic":reviewable,"blocked_needs_evidence":blocked_items,"physical_constraints":physical,"rejected":rejected,"rejected_rules":[{"rule":"self_loop","disposition":"rejected"},{"rule":"alias_or_synonym","disposition":"rejected"},{"rule":"legacy_relation_type","values":["contains","supplies_to","substitutes_for","transmits_to"],"disposition":"rejected"},{"rule":"dynamic_event_transmission","disposition":"rejected"},{"rule":"same_mechanism_input_and_dependency","disposition":"rejected"}],"qa":{"algorithm":"suffix-index O(sum(name_length)); no Cartesian product","no_full_pair_scan":True,"self_loops":0,"forbidden_relation_types":0,"write_ready_input_or_dependency":0,"write_ready_physical_constraints":0}}
    for item in reviewable: out["counts"]["approved_by_relation_type"][item["relation_type"]]=out["counts"]["approved_by_relation_type"].get(item["relation_type"],0)+1
