...
| 硬门语义 | 唯一详述来源 | 根入口/主 spec | Apply 验证锚点 |
|---|---|---|---|
| OpenSpec 生命周期、Proposal Review、Apply-final、Sync/Archive/Deliver | `.agents/openspec-workflow.md` | `AGENTS.md` 路由；主 spec lifecycle requirements | OpenSpec strict、workflow contract |
| Gate/package/checkpoint/commit、Gate Map、Complexity Budget | `.agents/openspec-workflow.md` | `AGENTS.md` gate 摘要；主 spec terminology requirement | task-design lint、proposal/tasks 对照 |
| Desktop-managed 入口与 fallback 双条件 | `.agents/git-workflow.md` | `AGENTS.md` Git hard gates；主 spec Git requirement | workflow architecture contract |
| sequential/parallel 分流与文件/写状态隔离 | `.agents/git-workflow.md` | `AGENTS.md` routing/hard gates；主 spec Git requirement | workflow architecture contract |
| Desktop 与 project-owned 两类 cleanup | `.agents/git-workflow.md` | `AGENTS.md` cleanup 摘要；主 spec Git requirement | cleanup order assertions |
| R0—R3、R2/R3 recovery、逐层写安全 | `.agents/openspec-workflow.md` | `AGENTS.md` 风险硬门；主 spec risk requirement | workflow contract、scope/recovery review |
| TDD、targeted/full validation、CI 触发边界 | `.agents/testing-tdd.md` | `AGENTS.md` TDD 摘要；主 spec validation requirement | architecture/contract tests、targeted tests |
| Skill 选择与 OpenSpec/Superpowers/GitHub 路由 | `.agents/skill-routing.md` | `AGENTS.md` routing table；主 spec Skill requirements | routing assertions |
| PostgreSQL 事实源、Neo4j 可重建投影、secret 与 prototype/doc 边界 | `AGENTS.md` 与 `openspec/config.yaml` 的稳定约束 | 主 spec safety/data requirements | scope/secret/link checks |

矩阵中的“唯一详述来源”不得在其他文件复制完整操作流程；根入口只提供不可绕过摘要，主 spec 只提供长期可验证行为。任何矩阵行缺失、来源交叉承载完整流程或对应检查失败，Apply package 必须停止。
