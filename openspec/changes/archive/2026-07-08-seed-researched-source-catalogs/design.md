...
前期调研的三个系统里，可进入观潮家的来源不是单一粒度：Vibe-Research 主要是策展 RSS 内容源；Vibe-Trading 主要是行情、RSSHub、loader provider 和部分 SDK source；Stock 同时包含新闻网页、Eastmoney 行情、指数、概念板块、Tushare、AKShare 和本地文件输出。这个 change 的核心不应只是“跑通一批 RSS”，而是建立统一来源目录能力，让系统后续能回答“我们一共接入了多少数据源、每个来源的用途、类型、provider、凭证、限流、连接器和状态是什么”。其中 SDK 类来源的接入和 worker/wrapper 边界已移出本 change，后续独立推进。

因此本 change 改为非 SDK 来源纳入 `source_catalogs`，真实采集按两阶段落地：内容/事件类来源优先可运行；Eastmoney HTTP 行情和板块类来源在同一 change 内建立可测试 connector/parser 边界。Tushare、AKShare 等 SDK 来源、凭证引用、worker/wrapper connector stub 和真实 Python SDK worker 均作为后续 change 的范围。
...
| Vibe-Trading | 非 SDK loader source 实际数量 | 读取 `/Users/meierlink/Documents/股票趋势验证/Vibe-Trading/agent/backtest/loaders/registry.py` 中 `VALID_SOURCES`，排除 `auto` 和 SDK-only loader，并在 report 中输出排除明细 |
| Stock | 非 SDK 来源条目实际数量 | 覆盖新闻网页、东方财富股票/指数/板块、本地历史文件等可治理来源；排除 AkShare、Tushare 等 SDK 类来源；允许 provider 级来源、endpoint 来源和代码列表型来源并存，但必须在 `source_config.kind` 中说明粒度 |

## Goals / Non-Goals

**Goals:**

- 将调研得到的内容类、HTTP 行情类、板块类和本地回灌类来源整理为 repo 内版本化 source seed 文件。
- 完成 Vibe-Research 108 条、Vibe-Trading 非 SDK loader source、Stock 非 SDK 来源的 seed 接入，并输出可审阅统计。
...
- 分阶段实现 connector 边界：内容源可真实运行，Eastmoney HTTP 行情/板块源具备可测试实现。
...
- 不在本 change 中登记或实现 Tushare、AKShare、Baostock、Futu、Mootdx 等 SDK 类来源、凭证引用、SDK 方法、worker/wrapper connector stub 或未实现 worker 错误。
...
本 change 将 Vibe-Research、Vibe-Trading 和 Stock 中可治理的非 SDK 数据源统一整理为 `source_catalogs` seed。来源类型至少包括：
...
- 板块类：Eastmoney 概念板块列表、板块 K 线、Stock 中预定义重点板块。
...
备选方案是只导入内容源。该方案短期更快，但会让行情和板块来源继续散落在调研结论和脚本里，无法统一统计、治理和审计。另一个备选方案是一次性真实实现所有 provider 和 SDK 采集，这会显著扩大外部依赖和凭证风险，不适合当前 change。
...
- `by_source_type`: 内容、行情、板块、本地回灌等分类数量。
- `by_status`: active、inactive、disabled 等状态数量。
- `by_provider`: provider 维度数量。

Vibe-Research 的 108 条配置和 106 个唯一 URL 是硬性验收；Vibe-Trading 需要输出排除 `auto` 和 SDK-only loader 后的实际非 SDK 数量；Stock 需要在 seed report 中给出非 SDK 来源实际数量和来源说明，因为 Stock 同时存在代码列表、样例股票、动态 provider、SDK 来源和历史输出文件，最终条目数由配置粒度和 SDK 排除口径决定。
...
现有表字段已经能表达通用来源身份，但 RSSHub route、网页解析策略、分页参数、分类标签、股票/指数/板块代码列表、市场范围、数据频率、字段映射、fallback 优先级等都不适合塞进 `source_url` 或 `topic_hint`。`source_config JSONB NOT NULL DEFAULT '{}'::jsonb` 用于保存来源专属的结构化参数。
...
新增 seed 和 repository 能力时，除了写入 active sources，还要支持按 `provider_key`、`ingest_channel`、`source_type`、`status` 和 `usage_policy` 统计来源。这样后续可以明确展示系统当前接入了多少来源、哪些可运行、哪些需要凭证或暂不可用。
...
- [Risk] 外部来源 URL 或 provider API 可用性可能变化。→ Mitigation：seed 测试只验证配置结构，采集单元测试使用 `httptest`、fixture 和 fake connector；真实连通性作为显式 smoke，不作为单元测试。
...
- [Risk] Stock 非 SDK 来源目标会因去重、代码列表合并、provider 级登记或 SDK 排除导致数量变化。→ Mitigation：seed report 必须输出实际数量、统计口径和差异说明；Vibe-Research 108/106 保持硬约束。
...
- [Risk] SDK 类来源被排除后，调研来源总数会低于原先全类型口径。→ Mitigation：seed report 必须输出 SDK 排除明细，并在后续独立 SDK change 中恢复治理。
...
3. 增加 repo 内 source seed 清单和 loader 测试，按内容、行情、板块、本地回灌分组。
4. 增加 seed command，把清单幂等写入目标 PostgreSQL，并输出来源统计。
5. 为内容源、Eastmoney HTTP 行情/板块源增加 connector/parser 测试。
6. 为 `IngestionJob` 增加并发执行测试，再实现 worker pool。
7. 使用 fixture/httptest/fake connector 验证多来源采集和 report 聚合。
...
- 本地 PostgreSQL 已执行 migration 和 source seed，seed report 输出 200 条新增调研来源，其中 Vibe-Research 108 条配置和 106 个唯一 URL、Vibe-Trading 13 条非 SDK loader source、Stock 56 条非 SDK 来源。
- 内容类 RSS 来源已通过 `source-ingest` 显式 smoke，并写入 `raw_documents`。
- Eastmoney HTTP connector/parser 已通过 fixture 单元测试；真实公网端点在当前网络返回空响应，未把失败伪装为成功。后续 `add-ingestion-scheduler` 或来源健康检查 change 需要把该类外部连通性纳入可观测状态、重试和禁用策略。

## Open Questions

- 非 SDK 来源 seed 后，哪些默认 `active`、哪些默认 `inactive` 或 `disabled`？默认建议内容源和可测试 HTTP 源可 active，其他需要凭证或高风险来源先 inactive。
