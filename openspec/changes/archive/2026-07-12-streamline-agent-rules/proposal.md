...
- 强化 Codex Desktop worktree 硬门：Desktop 机制可用时，新 change 和并行 change 必须通过 Desktop 新任务创建受管 worktree，agent 不得手工执行 `git worktree add`；只有 Desktop 机制不可用且用户明确批准 fallback 时，才允许创建项目自有 worktree。
- 将交付清理拆成 Desktop-managed 与 project-owned fallback 两条有序路径；Desktop 托管目录只能由 Desktop 释放，agent 不得对其执行 `rm` 或 `git worktree remove`。
...
- 不改变 OpenSpec artifact 路径、Skill 触发条件、`codex/<change-name>` 分支命名、数据库或运行时行为，不新增依赖；branch 在 Desktop 受管任务内创建或切换。
