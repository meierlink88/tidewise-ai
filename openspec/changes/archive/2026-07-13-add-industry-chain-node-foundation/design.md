...
首批6条`mapped_to_sector`采用Tidewise composite curation：证据由已审阅chain/node定义、canonical sector定义及来源、固定commit下的`mapped-to-sector-review.md`人工Review共同组成。外部sector定义或指数公告只证明目标板块边界，不冒充其直接声明chain/node mapping。现有`entity_edges`只有单一`source_name/source_url/verified_at/evidence_note`，因此`source_url`固定指向批准该组合判断的Review artifact commit，`source_name`标识Tidewise composite review，`evidence_note`登记组合证据及S5等外部方法来源角色；本change不为此扩表。
...
Physical constraint使用同一CLI的显式`-apply-scope industry-chain-physical-constraint`。scope后batch必须为Profiles/Memberships/TopologyEdges=`0/0/0`、PhysicalConstraints=`4`，不写entity/profile/relationship/sector mapping。正式记录保持`generated_by_ai=true`、`review_status=approved`，且每个ID必须在manifest携带显式`approved_by_human`并转换为仅存在于执行上下文的`IndustryChainApprovalGate`。Repository不得依赖同批membership/topology：写事务按稳定subject key排序，对node membership或topology edge执行`SELECT ... FOR SHARE`，确认同链active后才upsert constraint；锁与对应subject的更新/停用冲突，缺失或inactive时原子rollback。Physical constraint仍为PostgreSQL-only，不进入Neo4j。

Layer 6使用显式`-apply-scope industry-chain-sector-mapping`。完整manifest先通过既有loader/policy校验，scope后仅保留6条approved `mapped_to_sector`，不包含entity/profile/source mapping、industry-chain batch或其他relationship。Repository在单一PostgreSQL事务内按endpoint stable key稳定排序，以`SELECT ... FOR SHARE`锁定持久化active端点，校验`industry_chain|chain_node → sector`方向与policy，再逐条复用既有`UpsertRelationship`；relationship ID对应的from/to/relation_type为不可变identity，任一端点漂移、policy失败或identity conflict使整批rollback。Report预计created6/updated0/unchanged0且FinalTableImpact仅`entity_edges`。PG Write/Query必须另行授权；其验收完成后，Neo4j Rebuild与Query仍需新的独立授权。
