...
- 明确数据采集层的两种输入路径：自研爬虫脚本采集和外部 Agent API 采集结果接入。
- 明确采集数据的清洗、标准化、入库和后续进入关系型数据库、向量数据库或图谱数据库的演进边界。
...
- 不实现真实爬虫脚本、外部数据源抓取器、采集调度器或采集数据清洗流水线。
...
- `internal/ingestion`：采集流程、清洗、标准化、去重和入库编排。
...
### Decision: 数据采集层属于后端 ingestion 边界

数据采集层用于把外部热点事件、政策事件、市场异动、产业动态、公司公告、社媒/搜索热度和外部 Agent 已采集结果转化为系统可理解的数据资产。采集输入分为两类：

- 自研爬虫脚本采集：由本工程后端或后续独立采集服务通过 `jobs` 调度，通过 `integrations` 适配外部数据源，结果交给 `ingestion` 清洗和标准化。
- 外部 Agent API 接入：由外部 Agent 平台完成采集或初步分析，本工程通过 `integrations` 调用或接收回写，再交给 `ingestion` 做结构化校验、清洗、标准化和入库。

清洗和标准化后的数据可以按用途进入不同存储边界：

- 关系型数据：事件、来源、时间、标签、实体、市场指标、订阅触发和任务状态优先进入 PostgreSQL。
- 向量数据：文本切片、语义 embedding 和召回索引在 MVP 阶段优先由外部 Agent 平台承载，未来如引入向量数据库必须通过独立 change。
- 图谱数据：实体关系、事件传导链和板块/资产关系在 MVP 阶段先用 PostgreSQL 结构化关系表达，未来如引入图谱数据库必须通过独立 change。

选择理由：

- 数据采集是系统输入层，必须服务于后续 Agent 分析、报告生成、订阅推送和前端事件流。
- 自研爬虫和外部 Agent API 的数据质量、格式和时效不同，需要统一清洗标准化后再成为系统事实。
- 将采集编排放入 `ingestion`，将外部源适配放入 `integrations`，可以避免爬虫、Agent 回写和业务 handler 混在一起。

备选方案：

- 采集逻辑直接写入 `jobs`：调度方便，但清洗标准化和业务语义容易散落。
- 采集逻辑直接放在 `integrations`：外部源适配清楚，但无法承载去重、标准化、入库和任务状态编排。
- 一开始拆成独立采集服务：边界清晰，但 MVP 部署和调试成本更高。

### Decision: Agent 平台回写必须经过后端契约和幂等边界

Agent 平台可以由后端主动调用，也可以回写后端 API。所有回写必须经过后端鉴权、请求签名或密钥校验、幂等键、schema 校验、状态流转和落库。外部 Agent API 返回的采集结果也必须进入 `ingestion` 清洗和标准化边界，不得直接作为前端展示或长期事实数据。
...
- [Risk] 自研爬虫和外部 Agent API 会产生重复、冲突或质量不一致的数据。→ Mitigation：采集结果必须经过 `ingestion` 去重、来源追踪、置信度和标准化处理后才能入库。
...
4. 后续创建 `define-ingestion-architecture` 或在事件模块 change 中先补充采集契约，再实现自研爬虫和外部 Agent API 接入。
5. 后续创建 `define-api-contracts` 或在业务模块 change 中先补充契约，再实现真实 API。
6. 后续前端从 mock-first service 渐进切换到真实 API，不一次性替换所有页面。
...
- 自研爬虫首批采集源和外部 Agent API 的具体数据格式，需要在采集层 change 中结合合规性、频率限制和数据质量进一步确认。
