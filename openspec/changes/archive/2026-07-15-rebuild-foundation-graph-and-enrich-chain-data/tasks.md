...
- [x] 2.9 PG additive accepted baseline 验收后，按 [usable-map final relations Neo4j sync R3 Review 包](reviews/usable-map-final-relations-neo4j-sync-r3.md) 单次执行现有 `rebuild-entities`，完成 local Tidewise namespace cleanup+全量 rebuild；Neo4j 已验收为 981 nodes / 345 relationships=`133/108/3/93/8`，PG↔Neo missing/extra=`0/0`，PG 业务基线不变且 audit 为 `18/2`。脱敏证据见 [R3 execution 证据](reviews/usable-map-final-relations-neo4j-sync-r3-execution.md)；旧 checkpoint `e8b2658` 与旧 4-edge 包未调用。
...
- [x] 3.2 Apply-final Review 通过后，已将 3 份 delta specs 智能同步至对应 main specs，并归档到 `openspec/changes/archive/2026-07-15-rebuild-foundation-graph-and-enrich-chain-data/`；archive 后运行 `openspec validate --all`、主规格差异、归档路径与 active change 冲突检查，并提交 scoped archive checkpoint。
