...
- [ ] 2.1 在 `backend/internal/architecture/workflow_rules_test.go` 先新增或调整失败断言，覆盖 R0—R3、普通 checkbox 非人工 gate、阶段 Review package、R0/R1/Apply final 验证层级、R2 pre/post assertions、R2 逐层显式授权与失败后剩余授权失效、R3 独立授权、阶段级 checkpoint、agent self-review 和 active adoption 边界。
...
- [ ] 3.2 更新 `.agents/testing-tdd.md`，只维护 targeted/full 验证层级、R2/R3 pre/post evidence 和 self-review 测试证据边界，不复制完整授权流程。
...
- [ ] 4.1 运行 targeted `go test ./internal/architecture -count=1`、`openspec validate optimize-risk-tiered-development-workflow --strict`、规则文件链接检查、重复/冲突扫描、`git diff --check`、scope 与 secret 检查。
...
- [ ] 5.1 运行 Apply final 全量验证：`go test ./...`、OpenSpec strict validation、规则链接/重复/冲突检查、`git diff --check`、scope/secret 检查，并确认没有 R2/R3 操作或状态变化。
- [ ] 5.2 提交 scoped diff、全量验证证据、self-review 结果与阶段 checkpoint，等待主对话 Apply 后人工 Review；通过前不得 Sync、Archive、Deliver，也不得修改任何 active change tasks。
