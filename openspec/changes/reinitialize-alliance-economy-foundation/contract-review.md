# A：Schema / Data Contract Review（待主对话审批）

## 0. Review 状态与边界

本文是 tasks 1.1—1.3 的**待审契约**，不是已批准事实，也不是可执行 migration、seed 或数据库写入说明。基线为 2026-07-13 已 fetch 的 `origin/main@f942d7615afd952840cdc478bbe7b4ecc990616d`，仅审计现有 schema、entityfoundation loader/repository、seed JSON 与 graph projection 实现。

- 本文只准备 A 层 Contract Review；task 1.4 仍是未通过的人工门禁。
...
| 字段 | 建议类型与 null/default | trim/长度 | 唯一性与组合约束 | 与 `entity_nodes` 的关系 |
|---|---|---|---|---|
...
| `abbreviation` | `TEXT NOT NULL DEFAULT ''` | 写入前 `btrim`；建议 `char_length <= 32`；空串表示没有可审计正式简称，禁止 `NULL`、`—`、`-` 充当缺失值 | **不建议全局唯一**：不同组织可能同简称；非空值必须在同一 node 的 aliases 中恰好有一个 NFKC + Unicode casefold 等价值 | 保留原始展示大小写，不强制转大写；识别用 alias 规范化值，展示仍读 profile |
| `categories` | `TEXT[] NOT NULL`，**建议无 default** | 每项先 `btrim`、ASCII lowercase；单项建议 `1..64` 字符；数组建议 `1..8` 项 | 仅允许批准 allowlist；规范化后去重并按 code 字典序持久化；禁止空项、`/`、`、`、`|` 等拼接值；不得用父类自动补标签 | category 不是实体标签，不进入 aliases，也不改变 `entity_type`/`layer_code` |
| `leadership_summary` | `TEXT NOT NULL DEFAULT ''` | `btrim`；建议 `char_length <= 500` | 空串仅表示尚无可审计摘要；不得以“未知”“—”占位；写入候选时须携带来源供 Review，但来源不塞入本字段 | 只描述治理、轮值、秘书处或可审计主导方式；不得由文本自动生成 `led_by` |
| `influence_scope_summary` | `TEXT NOT NULL DEFAULT ''` | `btrim`；建议 `char_length <= 1000` | 空串仅表示尚无可审计摘要；不得保存评分或投资判断 | 只描述地理、议题或制度覆盖范围；不替代 categories、关系或 observation |
...
2. aliases 每项 `btrim` 后不得为空或为缺失占位符；建议单项不超过 128 字符、每实体不超过 64 项。
...
### 1.3 Categories 首版建议 allowlist

下表是**待审建议集合**。示例只说明分类边界，不代表任何具体联盟候选已获批准。
...
| `commodity` | 跨品类商品 | 跨多个商品大类的市场、生产者或治理协作 | 只有正式范围确实横跨能源、矿产、农业等多个商品族，且无法由单一窄类准确表达时使用 | 不作为 `energy`/`mineral`/`agriculture` 的自动父标签；单一商品组织不得仅用该宽泛 code | 跨商品生产者协调机制 |
...
1. **删除 `cross_domain`**：categories 已是多值数组，跨领域应直接存多个原子 code；保留 `cross_domain` 会同时表达“分类结果”和“分类数量”，含义不稳定。
2. **拆分复合 code**：建议将候选初稿的 `culture_education` 拆为 `culture`、`education`，`legal_dispute_resolution` 拆为 `legal`、`dispute_resolution`，`food_humanitarian` 收敛为 `humanitarian`；粮食生产/政策使用 `agriculture`。
3. **不自动补宽类**：出现 `trade`、`finance`、`development` 不自动补 `economic`；出现 `military` 不自动补 `security`；出现 `energy`、`mineral`、`agriculture` 不自动补 `commodity`。只有正式使命独立满足两个定义时才可多选。
4. **政治与治理分开**：`political` 表示政治/外交协调，`governance` 表示规则制定和公共治理。组织具有章程或理事会不构成 `governance` 准入依据。
5. **commodity 是受限兜底宽类**：建议保留但仅用于跨多个商品族的正式组织；若主对话认为该宽类仍不可审计，应从首版 allowlist 删除，而不是与窄类并用。
...
|---|---|---|---|---|---|---|
| `sovereign_state` | 经本项目权威 identity Review 认定为主权国家的 economy identity；ISO 代码本身不用于推断主权 | `economy:<alpha2 lowercase>` | `^[A-Z]{2}$` 且必须命中获批时点的 ISO 3166-1 alpha-2；与 key 后缀一致 | 建议 ISO 4217 alpha-3；多法定货币例外须逐项 Review，不能静默写 `MULTI` | 必须是受控 region，不能为 `global` | 可作为正式成员端点 |
| `territory_economy` | 具有独立 ISO 代码和统计/经济身份、但不等同于独立主权国家的地区经济体 | `economy:<alpha2 lowercase>` | `^[A-Z]{2}$` 且命中 ISO 3166-1；与 key 后缀一致 | 建议 ISO 4217 alpha-3；例外逐项 Review | 必须是受控 region，不能为 `global` | 仅在联盟官方成员来源明确把该 economy identity 列为正式成员时可建边 |
...
- `identity_kind`：建议 `TEXT NOT NULL`、无 default、仅允许上述四值；不允许用空值推断类型。
- `country_code`：写入前 trim + ASCII uppercase；建议 `NOT NULL`、`UNIQUE`。`EU` 与 `GLOBAL` 是内部保留 code，不得宣传为主权国家 ISO 代码。
- `currency_code`：写入前 trim + ASCII uppercase；主权国家/地区经济体通常是已审计 ISO 4217 alpha-3。`MULTI` 是内部保留值，不得作为缺失占位符。
- `region`：写入前 trim + ASCII lowercase。为兼容现有 50 条数据，首轮待审集合为 `africa`、`asia`、`central_asia`、`europe`、`europe_asia`、`global`、`middle_east`、`north_america`、`oceania`、`south_america`；是否把跨洲/政治区域进一步标准化不在本轮擅自决定。
- `entity_key`：稳定内部 identity，不因中文名、英文名、currency 或 region 变化而改变。国家/地区 key 必须与 alpha-2 code 一致；聚合 identity 只能来自显式保留表。
...
| 边界 | `origin/main` 当前状态 | 本契约建议目标 | exact diff / 风险 |
|---|---|---|---|
...
1. 批准 1.1 的五字段 null/default/trim/length/alias 组合规则，并确认 abbreviation 不做全局唯一键。
2. 批准 categories 为 1—8 个规范化原子 code、无 default、字典序持久化，且不自动补宽类。
3. 批准删除 `cross_domain`，拆分三个复合 code，并采用 1.3 的 23 个首版建议 code。
4. 批准 `commodity` 仅作为跨商品族受限宽类，不与单一窄类自动共存。
5. 批准 economy 四类 identity 矩阵、EU/GLOBAL 保留身份和 global 禁止 `member_of`。
6. 批准现有 10 个 region code 作为首轮兼容 allowlist；后续是否标准化另行 Review。
7. 批准明确不入库字段、无实体标签机制、Neo4j 单一 `Entity` label 边界。
8. 批准 exact diff 与 migration 风险作为未来 Apply/Write Review 的强制 preflight，但不视为 Apply 授权。

## 6. 需主对话决策项

以下任一项未决，都不得通过 task 1.4：

1. `categories` 是否接受 1—8 上限、无 default 与字典序持久化。
2. 是否接受删除 `cross_domain`，以及拆分 `culture_education`、`legal_dispute_resolution`、`food_humanitarian` 的方案。
3. `commodity` 是否保留为受限宽类；若不接受，应从首版删除，不能退回为任意兜底。
4. abbreviation 的 32 字符上限、aliases 的 128 字符/64 项上限，以及“不全局唯一、冲突进 Review”的规则。
5. 两个 summary 是否允许批准候选以空串暂存；若要求非空，需同时把 `DEFAULT ''` 改为无 default + 非空 check，并明确缺证候选的处置。
6. 主权国家/地区 economy 出现多个法定货币时，是否允许经逐项 Review 使用内部 `MULTI`；未决定前 validator 应 fail-closed。
7. `country_code` 是否建立全局唯一约束，以及 `entity_key` 唯一约束如何兼容未来 merge source 的稳定保留策略。
8. 现有 `europe_asia` 等 region code 是首轮保留还是在 C 层候选审计时迁移到新的区域体系；本轮不自行改写。

## 7. 本 checkpoint 的验收结论

tasks 1.1—1.3 已准备好供主对话 Review，但尚未批准。只有主对话对第 5、6 节逐项给出结论，才可勾选 1.4；在此之前不得开始联盟候选清单、economy 范围或关系候选。
