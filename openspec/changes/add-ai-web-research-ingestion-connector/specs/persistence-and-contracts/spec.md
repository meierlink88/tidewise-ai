...
- **THEN** PostgreSQL 必须保存该 source 的 search provider、搜索选项、LLM provider、API base URL、API 协议、模型名、提示词、提示词版本、时间窗口、结果上限、语言和输出 schema 等非敏感配置

#### Scenario: 引用真实凭证
- **WHEN** AI Web Research source 需要调用真实 Tavily API 或真实模型 API
...
- **THEN** 系统必须保留 search_provider、llm_provider、model、api_protocol、prompt_version、prompt_purpose、search_options、content_origin、retrieval_method、source_attribution_type、来源说明、Tavily 搜索元数据和原始返回片段等非敏感元数据
