...
系统 SHALL 将具备联网搜索能力的大模型 API 接入为 `source_catalogs` 驱动的采集连接器，并将检索结果标准化为原始文档候选对象。

#### Scenario: 执行 AI Web Research source
- **WHEN** 采集任务读取到 active 状态且 `connector_key=llm_web_research` 的采集源
- **THEN** 系统必须根据该 source 的 `provider_key`、`credential_ref`、`source_config` 和限流策略调用对应大模型 API 或 fake provider

#### Scenario: 缺少凭证引用
- **WHEN** AI Web Research source 需要 API key 但 `credential_ref` 为空或无法解析到真实 secret
...
- **THEN** 系统必须能够从 `source_config` 读取 API base URL、API 协议、模型名、提示词、提示词版本、搜索选项、时间窗口、最大结果数、语言和输出 schema
