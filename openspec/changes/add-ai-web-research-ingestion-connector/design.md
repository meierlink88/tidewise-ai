...
```json
{
  ...
  "tools": [
    ...
    {
      "provider": "searchapi_baidu",
      "role": "china_baidu_fallback",
      "max_results": 10,
      "credential_ref": "SEARCHAPI_API_KEY",
      "options": {}
    }
  ]
}
...
- `searchapi_baidu`: 百度 SERP 补充搜索工具，适合通过百度搜索生态补充中文来源；首期作为候选或 fallback provider，需要独立凭证和 adapter 测试。
...
```text
backend/data/prompts/ai_web_research/
...
2. 编写 Tavily、博查、SearchAPI Baidu adapter、prompt loader、LLM normalizer、parser 的配置校验、fake provider、结构化响应解析和 raw document candidate 转换测试。
3. 实现 `llm_web_research` connector、web search plan 编排、provider-neutral search adapter 接口、Tavily search adapter、博查 search adapter、SearchAPI Baidu search adapter、repo prompt loader、LLM normalizer、`llm_research_items` parser 和 registry 注册。
...
- Tavily 作为已验证默认 Web Search tool 保留；中国财经 source 需要优先验证博查和 SearchAPI Baidu 的真实搜索质量、成本、限流和凭证开通方式。
