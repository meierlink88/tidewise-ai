...
前期验证结论是：`Tavily Search API + DeepSeek/OpenAI-compatible LLM normalizer` 可以返回中国财经和全球政经候选材料，但单一 Tavily provider 对中国本土财经来源覆盖不稳定。因此首期设计必须支持多个 Web Search provider，并通过 source profile、来源偏好和可信域名白名单提高中国财经来源召回质量。
...
- 通过 `source_config` 保存非敏感运行参数：搜索 provider、provider profile、搜索选项、来源偏好、可信域名、LLM provider、API base URL、协议类型、模型名、提示词、提示词版本、结果条数、时间窗口、语言和输出 schema。
...
### Decision: 首期使用 Web Search Provider 抽象
...
首期 adapter 采用 provider-neutral 接口，并纳入以下 provider profile：
...
- `bocha_cn_finance`: 中文财经和中国本土来源优先 profile，适合召回中文网页、新闻、百科和生态内容；优先作为中国财经 source 的主 provider。
- `searchapi_baidu_cn`: 百度 SERP 补充 profile，适合通过百度搜索生态补充中文来源；首期作为候选或 fallback provider，需要独立凭证和 adapter 测试。

`source_config.search_provider` 表示 provider 类型，`source_config.provider_profile` 表示本 source 的用途模板。不同 profile 可以共享 connector，但拥有不同查询提示词、可信域名、语言、地区、时间窗口、结果条数和凭证引用。程序侧不得把 provider 返回的排名、摘要或 answer box 直接当成事实结论，必须进入 parser 校验和 raw metadata。
...
connector 内部使用统一请求模型表达 search provider、provider profile、LLM provider、protocol、model、prompt、search options、max results 和 timeout。Web Search provider 负责搜索召回；LLM 负责把搜索结果整理为 `items` JSON；parser 负责硬校验和 raw document candidate 转换。
...
- [Risk] 中国财经来源被全球搜索 provider 稀释。→ Mitigation：新增中文搜索 provider profile、可信域名白名单、来源偏好和结果后处理排序；中国财经类 source 默认不只依赖 Tavily。
...
- [Risk] Web Search provider 和 LLM 结构化成本、限流不可控。→ Mitigation：复用 provider 级限流，要求 `max_results`、timeout、搜索深度、批次大小、provider profile 和 fallback 策略可配置，并在 report 中记录搜索成功、搜索失败、LLM 成功、LLM 失败和跳过数量。
- [Risk] 模型总结被误用为事实原文。→ Mitigation：强制记录 `content_origin` 和 `retrieval_method`，后续事件抽取可按证据质量降权或跳过。

## Migration Plan

1. 定义多 Web Search provider + LLM AI Web Research source seed 示例和 `source_config` 字段约束。
2. 编写 Tavily、博查、SearchAPI Baidu adapter、LLM normalizer、parser 的配置校验、fake provider、结构化响应解析和 raw document candidate 转换测试。
3. 实现 `llm_web_research` connector、provider-neutral search adapter 接口、Tavily search adapter、博查 search adapter、SearchAPI Baidu search adapter、LLM normalizer、`llm_research_items` parser 和 registry 注册。
...
6. 通过 gated smoke 验证真实 Tavily、博查和 SearchAPI Baidu source 的搜索返回字段、错误处理和成本统计；LLM 真实调用同样必须显式启用，不得进入普通单元测试。
...
- Tavily 作为已验证默认 Web Search provider 保留；中国财经 source 需要优先验证博查和 SearchAPI Baidu 的真实搜索质量、成本、限流和凭证开通方式。
