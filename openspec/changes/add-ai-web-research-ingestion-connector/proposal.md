...
当前采集层已经能够通过 RSS、Eastmoney HTTP、本地回灌等固定来源写入 `raw_documents`，但缺少一种面向开放互联网的主动发现能力。为了捕捉过去 1 天内可能影响股票市场的全球政治经济热点，需要把 Tavily 这类明确返回 URL、标题、摘要和搜索元数据的 Web Search 工具接入采集流程，并用大模型对搜索结果做结构化整理。

本 change 只解决“Web Search 工具召回材料、LLM 结构化整理、采集层校验并入库候选原始文档”的问题，不做事件抽取、实体关联、图谱构建或投资判断。

## What Changes

- 新增 AI Web Research 采集 connector：由 `source_catalogs` 中的 active source 驱动，读取 `source_config` 中的 Tavily 搜索参数、LLM 结构化参数、提示词和输出 schema 配置。
- 首期使用 Tavily 作为 `web_search` 工具，由 Go 后端确定性调用搜索 API，获取 URL、标题、摘要、发布时间、来源名称和搜索元数据。
- 将 Tavily 与 LLM 的非敏感运行参数放入 `source_config`，包括 `search_provider`、`search_options`、`llm_provider`、`api_base_url`、`api_protocol`、`model`、`prompt`、`prompt_version`、`freshness_window`、`max_results` 和输出 schema。
...
- 将 Tavily 搜索结果和模型结构化输出标准化为原始文档候选对象，并通过现有 repository 幂等写入 `raw_documents`。
...
- 影响 `backend/internal/apps/ingestion/connectors`：新增或注册 `llm_web_research` connector，用于调用 Tavily Web Search、可选网页抓取和 LLM 结构化接口。
...
- 影响 `backend/data/source_catalogs`：需要新增 AI Web Research source seed 示例，配置 Tavily 搜索参数、模型接口、提示词、输出 schema、凭证引用和启用状态。
