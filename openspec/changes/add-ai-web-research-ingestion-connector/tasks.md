...
- [ ] 1.1 为 AI Web Research source seed 编写 loader/validator 测试，覆盖 `connector_key`、`parser_key`、`credential_ref`、`source_config` 必填字段、禁止敏感字段和状态。
- [ ] 1.2 定义 AI Web Research source seed 示例，包含 Qwen/OpenAI-compatible 类 provider 的 base URL、API 协议、模型、提示词、提示词版本、搜索选项、结果上限和输出 schema。
- [ ] 1.3 为 AI connector 配置解析编写单元测试，覆盖缺少 model、prompt、api_base_url、api_protocol、max_results、output_schema 和非法类型。
- [ ] 1.4 实现 AI connector 配置解析和校验边界，确保真实 API key 只能来自 `credential_ref`。

## 2. Provider 调用边界

- [ ] 2.1 使用 fake 或 `httptest` 编写 LLM provider 调用测试，覆盖成功响应、鉴权失败、超时、非 JSON 响应、空 items 和 provider 错误。
- [ ] 2.2 实现 provider-neutral 请求结构，使 connector 可以表达 provider、API 协议、模型、提示词、搜索选项、最大结果数和 timeout。
- [ ] 2.3 实现首个 OpenAI-compatible/Qwen-compatible provider 客户端边界，真实网络调用必须可被 fake/`httptest` 替换。
- [ ] 2.4 为凭证解析和请求构造编写安全测试，确保 Authorization header 不进入日志、raw metadata 或 source_config。
