...
- 增加 migration `000020`：为 `event_tag_defs` 增加精确的 `is_active`、`display_order`、`updated_at` 字段，增加 Tag assignment 审计字段（沿用已存在字段并补足约束），用固定 UUID literal + `INSERT ... ON CONFLICT` 内置 seed 22 条 Tag，创建最小 receipt 表、约束和索引；保留既有数据和 `event_entity_links`。
- 以确定性 UUID、英文稳定 `code`、中文 `name` 幂等 seed 22 个 active Tag：`news_category` 10 条、`index_category` 12 条；Tag 主数据是事件 CLI 校验和入库的必要支撑。
- 冻结 Agent source master manifest identity `tidewise:agent:event-reviewed-outbox`，按现有 `NormalizeUUID("source_catalog", manifest_id)` 得到稳定 UUID `b780800a-bd93-529c-b75a-ff2c4b16a0e5`；importer 固定 resolve 该 active source，不要求 payload 提供数据库 source ID/code，但保留输入真实 source fields。
- 固化 Tidewise v1 reviewed-outbox 输入契约：保留真实顶层 `idempotency_key`、`raw_documents`、单个 `event`、`event_sources`、`event_tags`、`review` 结构，只增补严格校验字段；明确 current-v0 的 `event_tags=[]`、`review.package_id` 缺失和 event source evidence metadata 不足到 required-v1 的最小差异；不修改 Agent。
