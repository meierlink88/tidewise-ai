...
- 提供可复用的 `EventImportService`，本地 CLI 是首个 adapter，未来 HTTP 可复用同一 service。
...
- 用稳定排序 active Tag 主数据生成 canonical JSON/hash，导出严格 JSON。
- 提供 migration forward recovery/down 兼容策略、TDD 边界、机器 JSON/exit code 和冻结命令示例。
...
### 4. migration、Tag seed 与 catalog canonicalization
...
Tag seed 使用固定 UUID（由 `tag_kind:code` 的版本化 UUID 派生且写入 fixture）、英文稳定 code、中文 name，22 条全部 active：news_category 10 条、index_category 12 条。upsert 只更新已声明的 name/display_order/active，不删除未知既有 Tag。Catalog 只读查询 active rows，按 `tag_kind`、`display_order`、`code` 排序；canonical payload 排除 `generated_at`，以稳定 JSON 字段顺序序列化后计算 `catalog_hash=sha256(canonical_payload)`，`catalog_revision` 为 migration/seed contract revision；输出时 `generated_at` 为 UTC RFC3339。
...
`cmd` 只解析 flags、装配依赖和输出 JSON；file adapter 只读文件；domain 不访问 DB/网络；repository 不解析 JSON、不读取环境变量；Tag catalog exporter 只读 repository；未来 HTTP 只调用 application service，不复制业务规则。
...
- [canonical hash 不稳定] → 明确排序、字段集合、UTF-8 JSON 与 SHA-256；`generated_at` 不参与 hash，并用 fixture 锁定。
