...
## 1. Event import and catalog implementation Package

- [ ] 1.1 固化 Tidewise v1 reviewed-outbox DTO、JSON schema、canonical payload hash、Review 状态/时间/evidence/tag validation，并为当前 outbox 兼容差异建立失败 fixture。
...
- [ ] 1.4 增加 `000020` migration SQL 与确定性 22-tag seed fixture；覆盖 active/display_order/updated_at、receipt schema/index/constraint、保留既有数据和 forward recovery/down 说明的静态测试。
- [ ] 1.5 增加本地 `event-import` CLI 与 `event-tag-catalog` CLI；覆盖单文件/目录、dry-run、strict JSON、exit code、stderr/stdout secret redaction 和 catalog canonical hash tests。
...
- [ ] 2.3 验证 receipt constraints/indexes、active tag count=22、canonical catalog hash、既有 raw/event/tag/event_entity_links 行数保留和 dry-run 零写入；保存 recovery/after assertions，不执行 UAT/prod/shared/Neo4j 或真实业务 import。
