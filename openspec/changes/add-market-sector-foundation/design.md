...
active-only 修复后的第二次 stateful rebuild 已得到 551 nodes、383 relationships 和 934/934 成功计数，但验收发现 Neo4j sector 节点没有 `classification_code`，29/23 只能由 key 前缀推导。修复继续复用单一 `GraphEntityNode -> GraphNode -> Neo4j Entity` 链：PostgreSQL 节点 source 对 `sector_profiles` 做 `LEFT JOIN`，以 `domain.SectorClassification` 承载可选真实值；sector 缺失或非法枚举由 mapper fail-closed，非 sector 字段保持空且 writer 清除残留属性。不得按 key 推导，不新增 profile 节点或平行投影。
...
- `000012` 应用后的普通 seed 复验又暴露 alias 全量排序导致 95 个无 convergence ownership 的非 sector 实体发生无意义更新。最终顺序规则固定为：正式 seed aliases 按首次出现顺序去重并保持原顺序；current audit-owned aliases 按 `moved_at,id` provenance 顺序去重，只追加正式集合中不存在的值。由于 local 已应用 `000012` 且其中两组多 alias sector 的字典序与 provenance 顺序不同，新增 forward migration `000013_normalize_current_convergence_alias_order.sql` 仅规范 current audit-owned target；后续先 apply `000013`，再由一次普通 seed 恢复 95 个正式顺序，第二次普通 seed 必须零更新。
