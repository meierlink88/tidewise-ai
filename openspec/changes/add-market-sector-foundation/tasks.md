...
- [ ] 6.4 TDD RED：增加 Memory/PG service/repository 测试，覆盖普通 seed fail-closed、显式 convergence、单事务 rollback、29 条 sector target 引用/legacy mapping、15 条 existing index target 类型安全迁移、16 条无 target retirement、未知 FK 阻断和 edge 冲突收敛
...
- [ ] 6.7 运行聚焦与 `go test ./... -count=1`，验证普通 seed 在 legacy active 时零写入、显式 convergence 结果为 52 active/60 inactive/60 audit/89 mappings/52 covers/0 tracked，重复执行幂等
