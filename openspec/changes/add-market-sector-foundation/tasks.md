...
- [ ] 1.2 按已确认权重计算候选评估：事件可解释性 25、传导独立性 20、行情敏感度 15、数据完整性 15、长期稳定性 15、市场代表性 10；六项分别依据 design 的独立 0/3/5 锚点取分，1/2/4 仅作锚点间中间档，并逐项保存 evidence/source，缺失项按 0 分
- [ ] 1.3 在 `openspec/changes/add-market-sector-foundation/candidate-review.md` 生成完整约 60 个候选 Review 清单，包含 canonical sector key 草案、source mapping identity、评分分项、总分、传导簇覆盖、合并/保留建议、benchmark 关联建议、核心/扩展/观察建议和 override 字段，并由 Git 版本记录保留本次来源快照历史
