...
- **WHEN** 候选中包含来源指数板块分类
- **THEN** 系统必须允许其作为 `index_sector` 进入 sector Review，并单独判别是否需要关联 benchmark、正式 index 或来源代码
...
#### Scenario: 校验来源分类
- **WHEN** seed loader 读取来自同花顺或其他来源系统的 `sector` profile
- **THEN** profile 必须保存可审阅的 `source_taxonomy_type` 或等价字段，区分 `concept`、`industry` 和 `index_sector`
...
- **THEN** seed 必须能够保留多个来源分类、来源代码、别名或映射记录，使审计可以追溯原始候选
