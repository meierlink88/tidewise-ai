...
- 不实现事件抽取、reasoning result、API、UI、数据库写入或 Neo4j rebuild。
...
- 同一端点对允许 `supplies_to` 与 `depends_on` 共存，但 evidence 必须分别解释供给事实和依赖事实；`substitutes_for` 不得与同方向 `supplies_to`/`depends_on` 共存。
...
AI/Agent 只能生成 candidate；缺少权威技术来源或人工 Review 的 candidate 不得进入 approved、正式 seed、Neo4j 或推理事实输入。
...
| `mapped_to_sector`（Review 候选名） | industry_chain/chain_node → sector | 经审阅的市场分析映射，不表达身份或法定覆盖 |

上一版 `represented_by_sector` 容易被理解为“板块代表产业链身份”，改名候选为 `mapped_to_sector`。最终命名仍需 Review；无论名称如何，都必须保存来源并明确是分析映射。海外 `market` 不得通过 `covers_sector` 指向中国 sector。
...
本 change 只保证静态表为后续 observation 提供稳定 chain/node/topology/physical constraint ID。未来 reasoning result 再组合事件、observation 与静态骨架，保存证据、时点、不确定性和证伪条件；不得回写主数据。
...
sequenceDiagram
    ...
    Projector->>PG: Read approved active static facts
...
- [`mapped_to_sector` 仍可能被误解] → 在 Review 清单中对比 `mapped_to_sector`、`associated_with_sector`，确认名称和方向后实现。
...
- sector mapping 最终命名采用 `mapped_to_sector` 还是更弱语义的 `associated_with_sector`？
- `supplies_to` 与反向 `depends_on` 是否允许为同一业务事实同时保存，还是只保留一个 canonical direction？
