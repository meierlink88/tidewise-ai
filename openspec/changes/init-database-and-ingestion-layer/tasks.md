...
- [ ] 4.1 新增 PostgreSQL repository 接口和实现边界，覆盖 `source_catalogs` 读取、`raw_documents` 幂等写入和采集状态更新。
...
- [ ] 5.1 新增 `backend/internal/ingestion` 包，建立采集编排服务和标准化数据结构。
...
- [ ] 6.1 实现 `rss_feed` connector 和 `rss_item` parser，覆盖标准 RSS/Atom 标题、链接、摘要、发布时间和正文候选内容。
- [ ] 6.2 实现 `http_eastmoney` connector 和 Eastmoney JSON parser，统一 User-Agent、timeout、限流和错误处理。
- [ ] 6.3 实现 `rsshub_feed` connector，支持 `RSSHUB_BASE_URL`、`route_template`、`code_style`、timeout 和安全 XML 解析。
- [ ] 6.4 实现 `web_fetch` connector 和 HTML/text parser，保存原始对象并提取可读正文。
- [ ] 6.5 实现 `local_file` connector 和 CSV/JSON/text parser，用于本地历史材料回灌。
- [ ] 6.6 为 `sdk_tushare` 和 `sdk_akshare` 建立声明式 connector stub，明确需要后续 Python worker、sidecar 或内部 HTTP wrapper 执行真实 SDK。
