...
- [x] 7.1 在 `backend/internal/jobs` 中新增采集任务入口或任务接口，用于按采集源批量执行 ingestion。
- [x] 7.2 添加采集任务错误分类、重试建议和部分失败处理，确保单一来源失败不阻断整批任务。
- [x] 7.3 确认采集层不生成影响方向、评分、传导强度、预测结论或投资建议。
- [x] 7.4 添加最小 seed 或 fixture 策略，用于本地验证采集源目录和连接器注册，不包含真实密钥。
