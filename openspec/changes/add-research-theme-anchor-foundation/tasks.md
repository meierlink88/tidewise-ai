...
- [ ] 1.2 逐项决策 `impact_level`、`transmission_stage`、`relation_role`、`importance`、`evidence_role`、`impact_direction`、`anchor_type` 候选集合，以及 `published_at`、分析窗口可空性。
- [ ] 1.3 确认 non-goals、active change 隔离、PostgreSQL-only、无 research conclusion/run/display order/Neo4j 结构的边界，并记录 Review 结论后再开始 Apply。

## 2. Schema And Persistence Package

- [ ] 2.1 先编写 schema contract、领域校验和迁移解析测试，覆盖 8 张表、必填文本、受控 CHECK、时间窗口、FK 删除行为、复合唯一键和无 `display_order`。
- [ ] 2.2 编写单个 forward-only `backend/migrations` migration，按 design 创建 8 张表、审计字段、明确 FK、索引和 Proposal Review 已确认的受控集合；不插入主数据。
- [ ] 2.3 在 `internal/domain`/`internal/repositories` 中仅实现后续回写确有必要的类型/访问边界，保持 PostgreSQL repository 方向；用 Go fake/SQL mock 覆盖成功、非法枚举、空白文本、窗口错误和 FK 错误。
- [ ] 2.4 在获批的 local R2 layer 内执行 preflight → migration → schema assertions，确认既有 events、chain_node_profiles、index_profiles 计数不变；不得执行 UAT/prod/Neo4j/seed/业务写入。

## 3. Apply-Final Verification Package

- [ ] 3.1 运行受影响 backend persistence/schema 边界的完整 suite、精确 OpenSpec task-design lint、`openspec validate add-research-theme-anchor-foundation`、`git diff --check` 和 secret/scope 检查。
- [ ] 3.2 完成 self-review：逐条核对 proposal/design/spec/tasks 与实现一致，复读 fresh verification 输出，记录未验证项/风险；提交 scoped `spec: propose add-research-theme-anchor-foundation` checkpoint 后等待 Apply-final Review。
