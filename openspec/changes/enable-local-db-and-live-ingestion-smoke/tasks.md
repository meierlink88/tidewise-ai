...
- [x] 5.1 增加本地 PostgreSQL 运行模板或说明，明确必需环境变量、数据库名称、用户、password 注入和不提交 `.env.local` 的规则。
- [x] 5.2 增加 smoke 运行说明，包含迁移命令、采集 smoke 命令、复跑幂等验证和常见失败排查。
- [x] 5.3 检查新增模板、说明和测试 fixture，确认没有真实 secret、生产连接串、个人 token 或私有凭证。
