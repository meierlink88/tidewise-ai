...
- [ ] 2.1 设计 repo 内实体 seed 文件格式，并编写 loader/validator 测试，覆盖必填字段、重复实体 key、重复关系、悬空 profile、悬空关系和禁止推理结论字段。
- [ ] 2.2 实现实体 seed loader 和 validator。
- [ ] 2.3 编写联盟组织 seed 数据，至少覆盖 `OPEC+`、`OPEC`、`G7`、`G20`、`WTO`、`IMF`、`World Bank`、`OECD` 和 `EU`。
- [ ] 2.4 编写经济体 seed 数据，覆盖中国、美国、香港、全球等一阶段基础经济体。
- [ ] 2.5 编写政策机构 seed 数据，覆盖中美核心央行、财政、监管和宏观政策机构。
- [ ] 2.6 编写市场和指数 seed 数据，覆盖 A 股、港股、美股、加密市场和核心宽基指数。
- [ ] 2.7 编写板块、产业链节点、指标和商品 seed 数据，覆盖一阶段基础分类、通用链路节点、常用行情指标和宏观传导标的。
- [ ] 2.8 编写公司、证券、交易工具和人物 seed 数据，使用少量基准样例完成所有 profile 表初始化验证，不导入全量股票池。
- [ ] 2.9 编写实体关系 seed 数据，覆盖联盟成员、经济体市场、市场指数、公司证券、人物机构、指标适用对象等客观关系。

## 3. Repository、service 和命令入口

- [ ] 3.1 为实体 seed repository 编写 fake 和 table-driven 测试，覆盖实体节点、profile、关系的幂等 upsert。
- [ ] 3.2 实现实体 seed repository 或复用既有 repository 边界完成实体、profile、关系写入。
