...
系统 SHALL 以 `(source_system, source_taxonomy_type, external_code)` 识别唯一外部 identity，并使用可扩展文本规范值而不是 PostgreSQL enum 锁死来源或 taxonomy。
...
- **AND** source_taxonomy_type 必须逐代码规范为 `industry_sector`、`concept_sector` 或 `index_sector`
...
系统 SHALL 只为已批准的 842 个第一批 chain_node 准备 1,156 条东方财富/同花顺外部标识，不得顺带迁移旧 sector mapping、其他实体或未经 Review 的来源。

#### Scenario: 拆分工作簿来源代码
- **WHEN** data contract 工具读取已批准工作簿
- **THEN** 必须把来源代码拆成 1,156 条逐行记录，其中 eastmoney 811 条、ths 345 条
- **AND** 必须验证 241 个节点同时具有两侧代码
- **AND** 必须验证不存在一个外部 identity 指向多个 canonical node

#### Scenario: 恢复逐代码名称和 taxonomy
- **WHEN** 标准化节点 row 聚合了多个原始名称或来源代码
- **THEN** 转换必须从来源侧明细恢复每个 external_code 对应的 external_name 与 source_taxonomy_type
- **AND** 不得把聚合的组合分类字符串直接写入 source_taxonomy_type
- **AND** 当前 6 条组合分类记录涉及的 13 个代码未逐代码消歧时，dry-run 必须失败
...
- **WHEN** 842 个 node/profile Query 已验收且 1,156 条 mapping report 已明确授权
- **THEN** 系统才可写入 mapping data
- **AND** Write 后必须立即 Query 总数、provider counts、双来源节点数、taxonomy/name 完整性、唯一性、entity 绑定、孤儿与幂等
