...
def test_candidate_contract_and_determinism():
    ...
    assert data["counts"]["approved_semantic"]==96
    assert data["counts"]["blocked_needs_evidence"]==51
    assert data["counts"]["rejected"]==7
    assert data["counts"]["physical_constraint_blocked"]>0
    assert data["counts"]["approved_by_relation_type"]=={"is_component_of":1,"is_subcategory_of":95}
    assert data["counts"]["blocked_by_relation_type"]=={"depends_on":1,"input_to":3,"is_subcategory_of":47}
    assert data["qa"]["self_loops"]==0
    assert data["qa"]["write_ready_input_or_dependency"]==0
    assert not ({"contains","supplies_to","substitutes_for","transmits_to"} & {x["relation_type"] for x in data["approved_semantic"]})
    assert all(x["disposition"]=="ai_approved_candidate_draft" and x["mechanism"] and x["derivation_evidence"] and x["counterexample"] for x in data["approved_semantic"])
    approved={e["key"] for e in json.loads(g.INPUT.read_text())["entities"]}
    review_tuples=[]
    for item in data["approved_semantic"]:
    ...
    rejected_directions={x.get("direction") for x in data["rejected"]}
    assert {"稀土产业 -> 稀土永磁","铜产业 -> 铜缆高速连接"} <= rejected_directions
