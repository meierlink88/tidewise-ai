...
## 首次执行结果（fail-closed，已保留）

同一窗口 preflight 满足 Goose=16、466 entities、331 edges、0 chain_node/profile/external identifier、0 writer/long transaction，以及 stable backup SHA-256/size。标准 `entity-seed -manifest-file` 在首个 entity upsert 被 PostgreSQL 报错阻断：`entity_convergence_alias_moves` 不存在。立即只读 Query 确认 Goose=16、`entity_nodes=466`、`entity_edges=331`、`chain_node=0`、`chain_node_profiles=0`、external identifier=0，未发生部分写入。根因与 R1 最小修复见 [forward-fix Review](phase-a-chain-node-seed-forward-fix-review.md)。

## 经重新授权的 R2 执行结果

主对话随后明确批准 R1 修复验证通过后的单次 R2 重试。刷新 post-cleanup preflight 后，唯一标准 Write 成功创建精确 manifest 的 842 个 `chain_node` 与 842 个 `chain_node_profiles`；没有 external identifier mapping、theme、relation、migration 或 Neo4j 操作。完整脱敏 preflight、Write 和 Query/assert 证据见 [phase-a-chain-node-seed-execution-evidence.md](phase-a-chain-node-seed-execution-evidence.md)。本结果仍须独立人工验收，且不构成 mapping 层授权。
