...
同一窗口 preflight 满足 Goose=16、466 entities、331 edges、0 chain_node/profile/external identifier、0 writer/long transaction，以及 stable backup SHA-256/size。标准 `entity-seed -manifest-file` 在首个 entity upsert 被 PostgreSQL 报错阻断：`entity_convergence_alias_moves` 不存在。立即只读 Query 确认 Goose=16、`entity_nodes=466`、`entity_edges=331`、`chain_node=0`、`chain_node_profiles=0`、external identifier=0，未发生部分写入。不得重试、不得手工 SQL；需先以独立 forward-fix Review 处理残留逻辑引用。
