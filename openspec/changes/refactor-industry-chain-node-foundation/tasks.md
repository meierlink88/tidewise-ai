...
- [ ] 1.10 展示最新 schema Write preflight、影响表/row 范围、备份验证、事务与 forward-fix 回滚边界，单独请求 PostgreSQL schema Write 授权；获批后 apply Phase A migration，并立即 Query schema、约束、版本与重复执行结果。
- [ ] 1.11 在 schema Query 验收后，再次展示节点初始化影响并单独请求 PostgreSQL data Write 授权；获批后只写入批准的 chain_node，立即 Query counts、profile 完整性、重复、孤儿、stable references、created/updated/unchanged/failed 与幂等结果。
...
- [ ] 2.8 展示最新 relation schema Write preflight、影响、备份与回滚边界，单独请求 PostgreSQL schema Write 授权；获批后 apply migration，并立即 Query 表、约束、索引、FK、旧结构状态与重复执行结果。
- [ ] 2.9 在 schema Query 验收后，再次展示候选边/constraint 影响并单独请求 PostgreSQL data Write 授权；获批后只写入已批准关系和 subject 映射，立即 Query 方向、端点、tuple、自环、机制冲突、evidence、孤儿、未迁移阻断项与幂等结果。
- [ ] 2.10 如需停用或移除旧 `sector_profiles`、`sector_source_mappings`、`industry_chain_profiles`、`industry_chain_memberships`、`industry_chain_topology_edges`，先再次提交引用扫描、备份与影响并取得独立 PostgreSQL Write 授权；只能用版本化 forward migration，禁止手工清库或历史回滚。
