...
- [ ] 1.11 **First-batch data contract Review 门禁**：主对话审阅 842 节点范围、aliases 规则、definition/boundary 生成与逐项 Review 策略、全新 UUID/entity_key、去重/幂等、dry-run/report，以及通用 `entity_external_identifiers` schema。6 条组合来源分类记录涉及 13 个代码，逐代码 taxonomy 未消歧前不得批准可执行 seed；不得确定具体 theme 实例。
- [ ] 1.12 **Schema/TDD implementation Review 门禁**：仅在 1.11 明确获批后，测试先行实现 `entity_external_identifiers` migration/domain/repository、节点 identity、final seed dry-run/report 与逐行映射解析；提交 scoped diff、测试、schema diff 和 dry-run 格式供 Review，不 apply migration、不写数据库。
