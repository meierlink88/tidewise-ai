...
- [x] 1.15 **R2 条件式执行包：external identifier schema -> Write -> Query/assert**：cleanup Query 已经 checkpoint `f2bc90a` 验收后，提交独立 [R2 authorization package](phase-a-external-identifier-schema-authorization.md)，包含当前 local 身份/Goose=15/16 pending、migration 16 精确 schema diff、fresh post-cleanup backup/archive evidence、零 data rows 范围、before/after assertions、target-version=16 唯一路径与 fail-closed 停止条件。package 已准备但尚未授权或执行；不授权 node/profile、mapping、relations、migration 17、Neo4j 或 UAT/prod。
