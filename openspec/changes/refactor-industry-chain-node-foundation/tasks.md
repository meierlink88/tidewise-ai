...
- [ ] 1.12 **Schema/TDD implementation Review 门禁**：checkpoint `775afda` 与首轮 remediation checkpoint `12820f9` 均未通过，整改复验前保持未完成。仅在 1.11 明确获批后，测试先行实现 `entity_external_identifiers` migration/domain/repository、节点 identity、final seed dry-run/report 与逐行映射解析；必须具备 CLI→Service→Executor 可审计的 migration target-version、同一 pinned connection 的 advisory lock lifecycle、锁内 before/after 状态推导的实际 applied/remaining、aliases trim/去重/稳定排序、完整 node/mapping snapshot 幂等与冲突判断、wide-boundary=79 计数，以及并发换绑必然失败的事务 repository 契约；提交 scoped diff、测试、schema diff 和 dry-run 格式供 Review，不 apply migration、不写数据库。
