...
- [ ] 1.15b **R2 条件式执行：external identifier schema Write -> Query/assert**：仅在 1.15a package 获主对话明确授权、同一维护窗口 preflight/recovery evidence 全部通过后，才可执行 migration 16 schema Write，并立即 Query/assert 表、列、FK、唯一约束、索引、版本与零 data rows；不得执行 migration 17、node/profile、mapping、relations 或 Neo4j。
