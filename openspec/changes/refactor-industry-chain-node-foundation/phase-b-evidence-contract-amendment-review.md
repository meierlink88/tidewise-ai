...
该 amendment 只改变证据充分性判断，不改变四类 relation 语义、数据库字段、方向、tuple unique 或 R2 人工授权门禁。用户确认前不得准备或执行 `phase-b-relation-schema`。
...
1. 用户确认或修改本 evidence contract amendment。
2. 才可准备 task 2.6 relation schema R2 package；schema Write 与 Query 仍需明确授权。
