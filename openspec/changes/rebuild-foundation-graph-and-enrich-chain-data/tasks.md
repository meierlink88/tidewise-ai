...
- [x] 2.6 按 usable-map 语义对 842 个节点完成 additive 候选发现与双遍检查；逐行保护既有 100 条，允许无事实节点保持无边，生成输入指纹、规则、coverage、Tier 1/Tier 2 manifest、blocked/rejected、分布与 hash 验证 Review 包。本 checkpoint 不授权任何写入，见 [842-node usable-map additive R0 Review 包](reviews/chain-node-relations-usable-map-r0/README.md)。
- [ ] 2.7 仅在 R0 additive manifest 独立 Review 冻结后，评估现有固定 manifest contract 是否需要最小 R1 path/hash/count 更新；若不需要则记录复用证据，禁止扩大为通用框架。
