...
最新 workflow/lint 仍不允许 R3 使用 `approved-disposable-recovery`。本 change 保留真实语义，不伪造 Neo4j backup，不修改 workflow；explicit task-design lint 因此预期失败，且 `allow-local-neo4j-disposable-r3-recovery` 完整 Deliver 前不得 Apply。
