...
- [ ] 2.1 提出一个有限、可关闭的首批产业链范围，说明选择理由、起始方向、包含/排除边界和停止条件，等待用户业务 Review；本 Proposal 不预设产业方向、entry node、hop 或节点/关系数量。
- [ ] 2.2 范围批准后，从已有节点向下探索有限细分 chain_node；用一遍 AI 生成四类关系候选、来源、证据、反例、置信度和 disposition，再由主对话 double-check identity、端点、方向、证据与边界。
- [ ] 2.3 展示完整候选与异常/冲突项，等待用户 Review 并冻结 final 节点/关系候选；候选分析不推定任何 Write 授权。
- [ ] 2.4 默认作为纯数据任务复用现有 schema、写入能力与 graph-projector，不改源码、不新增测试、migration、repository/service、runner 或 dry-run/report framework；只读能力审计若发现硬缺口，必须带证据回到 Review。
- [ ] 2.5 准备并等待 `first-batch-postgres-write` 独立 R2 授权；获批后在现有事务边界内原子写入 final 候选，立即 Query 端点、tuple、orphan、范围外保护与幂等，任何漂移、冲突或范围外变化立即停止。
- [ ] 2.6 只在 workflow schema blocker 已经独立解决且 PG Query 验收后，准备并等待 `first-batch-neo4j-sync` 单独 R3 授权；获批后只读 PG 同步 Neo4j 并验收 counts/type、missing/duplicate/orphan/legacy=0。
