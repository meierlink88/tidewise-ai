...
- [ ] 2.8 等待 `usable-map-additive-relations-postgres-write` R2 Write 授权；获批后仅 additive 写入新关系，逐行保护既有 100 条并立即 Query 类型、端点、tuple、orphan、范围外保护与幂等。[独立 R2 Review 包](reviews/usable-map-additive-relations-postgres-write-r2.md) 已通过；fresh preflight 与一次 full backup 见 [execution-preflight 证据](reviews/usable-map-additive-relations-postgres-write-r2-execution-preflight.md)。尚未执行 Write、restore 或 Neo4j，task 保持未完成。
