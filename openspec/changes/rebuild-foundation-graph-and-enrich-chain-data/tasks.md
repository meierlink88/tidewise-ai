...
- [ ] 1.3 冻结 PG projection baseline，准备并等待 `local-neo4j-foundation-cleanup` R3 授权；获批后仅清 local Tidewise namespace，不做 Neo4j backup/rollback，并立即 Query 为零。授权包见 [local Neo4j 基础投影 cleanup R3 授权包](reviews/local-neo4j-foundation-cleanup-r3.md)；当前仅完成只读准备，未授权、未执行。
