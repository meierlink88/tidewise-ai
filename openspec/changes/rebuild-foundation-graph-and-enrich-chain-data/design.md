...
第一遍按关系语义和方向整理候选，第二遍使用 Serenity 方法检查机制、替代路线、反例和宽边界；physical constraints 不进入本 change。Tier 1 要求可信外部来源支持链路，允许同一权威产业链资料支持一组相关边；Tier 2 允许暂缺外部来源，但两遍必须对类型、方向、机制、条件和反例完全一致，并在现有 evidence_note/provenance/condition_note/verified_at/status 字段标记 AI knowledge double review。这是工作方法，不新增 capability、字段、服务或审核平台。

`input_to` 定义为 A 的产出沿可解释产业链进入 B 的生产、产品、交付或运行过程，允许跨越当前 842 节点中缺失或未建模的中间环节，方向仍为上游 A → 下游 B。名称邻近、市场相关、共同事件或宽泛行业相邻不能构成关系。
...
- [宽化 input_to 导致泛化补边] → 每条边必须有可解释机制、条件和反例，宽边界候选进入 blocked ledger。
