...
- [ ] 1.3 建立关键规则覆盖矩阵，至少覆盖 OpenSpec 唯一生命周期、Desktop 原生任务/worktree、新 change 隔离、Review/Apply/数据库写入/图谱分层审批、sync/archive/deliver、merge 后 branch/worktree/Desktop 任务清理、PostgreSQL/Neo4j、不混 change、prototype、secret 和投资建议边界。
...
- [ ] 2.3 精简 `.agents/git-workflow.md`，使其成为最新 `origin/main`、`codex/<change-name>`、Codex Desktop 原生 worktree、scoped commit、PR/merge、远端/本地 branch、项目自有 worktree 与 Desktop 任务归档的唯一完整来源。
...
- [ ] 4.2 检查根路由表、规则文件和 OpenSpec artifacts 中引用的 repo 内路径均存在，确认不存在失效链接或要求无差别读取全部规则的残留文本。
- [ ] 4.3 运行 `openspec validate streamline-agent-rules`，检查 `git diff --check`、`git status --short` 和 scoped diff，确认只包含本 change 允许的规则与 artifacts，且未触碰其他 active change/worktree。
- [ ] 4.4 提交精简后的规则实现前，将前后尺寸、覆盖矩阵、重复/冲突扫描、链接检查、OpenSpec validate 和 scoped diff 交由用户人工 Review；未批准不得进入 Sync、Archive 或 Deliver。
