...
- `Recovery Evidence` 只能是 `backup` 或 `approved-disposable-recovery`。后者保留 local R2 的既有支持，并仅为 local R3 增加一个窄例外：Gate 必须为 `Risk=R3`、`Human=yes`，Layer 或 Scope 必须包含大小写不敏感且有 ASCII token 边界的 `Neo4j`，并包含同样有边界的 `cleanup`、`rebuild`、`sync` 之一，Before Assertions 必须包含有边界的 `PG` 或 `PostgreSQL` 以及 `baseline`。这些机器锚点只允许表达 recovery，不构成 R3 执行授权。`Recovery Baseline` 只能是 `new:<kebab-id>` 或 `reuse:<kebab-id>`。
