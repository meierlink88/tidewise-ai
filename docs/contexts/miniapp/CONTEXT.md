...
- 顶部展示“全球政经事件”和“读懂全球政经变化”；日期、星期取当前时间，按上海时区展示，页面返回前台和跨日时更新。
  “截至 MM.DD HH:mm”来自所选 Report 的 `publishedAt`，按上海时区展示。没有 Report 或发布时间无效时隐藏截至标记，仍展示当前日期。
...
- 分享回调不主动调用分享菜单 API，不调用受限导航栏 API；详情标题保留在页面配置中供系统单页导航使用。
- 普通模式首页与详情共用 NavigationBar 自定义导航，标题统一 17px、600 字重、系统字体和正常字距，不应用正文放大。详情返回优先回上一页，直接打开时回首页；失败提示重试。朋友圈单页两页均隐藏自定义导航。
...
- 产业链节点方向标签：升温红色、降温绿色、分化黄色，直接依据节点白底高亮，选中状态独立于判断来源。核心分析采用浅色外框、白色内卡，显示选中节点名称、方向、affected_nodes[].assessment.conclusion；下方“关键信号”标题下逐项 bullet 只展示同一节点 variable_signals[].signal 原文，不加变量名称与方向。可选信号缺失时隐藏标题和列表，不补造、不借用整链信号。无涨幅、周期、置信度、公司卡片或验证时间线。内容核对基线为 uat-trial-005 报告，tidetell1.0 只提供视觉样式。
- 白色推理底板到支持/反证结束；下方淡黄色圆角描边区域依次展示“产业链图谱”、节点图谱和“核心分析”。保留中央示意连接线，左右边缘与上方白色底板对齐，核心分析内部结论卡保持白色。
- 节点卡片采用名称、结论方向、变量信号摘要三层布局；底部逐项显示同一节点 variable_signals 的变量名及 source_direction，多个变量换行，卡片高度随内容适配。信号正文仍仅在核心分析展开；不补充上中下游数据。
- 首页、报告详情和 Evidence 子页面的内容字号统一通过 report-reading-font 按原字号乘 1.3，保留层级比例及原无单位行高。仅放大文字，不同比缩放图标、边框或图谱固定宽度；卡片与标签通过换行适配。系统原生导航栏字号不由此函数控制。
- 试读后调整：首页卡片结论、详情顶部结论、本链结论、关键机制正文、节点结论恢复放大前字号；详情 Tab 名称为 24rpx、分类为 16rpx，均小于放大前。其余阅读字号保持 1.3 倍。核心分析节点卡片采用浅色统一底、灰色细边框、左侧金色渐变竖线，取消内部嵌套白色结论卡。
...
我的页保留首页头像入口，头像和登录/注册或当前昵称居中，下方仅提供关于观潮家入口，无余额与服务网格。
底部菜单为推理/跟踪，跟踪页暂空。点击登录/注册进入独立登录页，成功返回我的；
点击已登录身份进入资料编辑，退出与编辑分离。关于页展示品牌与真实发布版本（无版本时不编造），底部进入隐私政策；登录页也可打开政策。
一键登录使用 wx.login，不要求手机号；次入口使用 getPhoneNumber 原生授权并同时获取 wx.login code。
两入口共用默认未勾选的隐私政策，点击政策链接不会勾选或发起登录；未同意时不调用平台登录。
前端显式传递当前 privacy_version，BFF 将 phone_code 和版本转交 User；不接收或处理明文手机号。
抖音/H5 不提供微信登录能力。会话期限、游客报告读取与既有导航不变。

## 私有头像与账号视觉（#530）

个人资料通过微信 chooseAvatar 与 input type=nickname 主动填写，form submit收集最终昵称，
头像在保存时读取临时文件上传，与昵称原子提交。取消不写入，失败保留草稿。头像专用读取
经BFF授权，微信端转为用户ID隔离的两槽本地图片缓存用于展示；游客恢复默认头像。
我的、登录、资料编辑、关于共用首页深蓝顶部、米白圆角内容区、深蓝主按钮和淡金点缀。

## 公司跟踪（#535）

跟踪页以公司全称为标题，行业和主题概念为标签，股票简称/代码为补充。
搜索仍按股票代码、简称或拼音首字母。无行情、市场Tab、管理模式或标签权重。
Miniapp tracking Biz 编排 Data stock 查询和 User watchlist 私有接口；
BFF 仅投影 title（full_name 为空时用 name）、industry_label/path 与跟踪状态，
不保存关系或领域事实。列表批量查资料，缺失引用或下游失败显式报错，不删除关系。
游客可搜索，添加前登录；写入只使用当前 session。删除不依赖 Data 可用性。
Frontend feature Port 校验 wire，250ms防抖，请求代次/会话代次防止旧响应覆盖；
成功写入后刷新真实列表与计数。取消需原生确认，公司详情展示全部来源标签。
公开合同 /api/miniapp/v1/tracking 及 /search、/{stock_id} 见 OpenAPI。
