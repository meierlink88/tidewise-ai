...
一个 Theme 的推理树集合必须对该 Theme 的每个 Theme Chain Node Association 恰好提供一个以该节点为中心的 Research Anchor，中心节点集合不得缺失、新增或重复。任一 Anchor 缺失或无效时，该 Theme 的 Anchor 集合整体不发布，并保持无可见推理树；待补树 Theme 可作为技术暂存保留，但不属于首页产品可见 Theme。
_Avoid_: 部分 Anchor 可见、额外 Anchor Tab、一个中心节点对应多个 Anchor

**产品可见 Theme（Product-visible Research Theme）**:
同时拥有成功 Theme Publication 和成功 Research Anchor Publication Receipt 的 Theme。首页 Theme 列表只返回这类 Theme，因此每个列表项必然至少拥有一条推理树，不需要 `has_reasoning_tree` 布尔字段。可见性按单个 Theme 判定：同一分析批次内已完成 Anchor 发布的 Theme 可立即展示，不被同批待补树 Theme 阻塞；首页 Theme/Event 计数只基于当前可见子集计算。
_Avoid_: 在首页暴露待补树 Theme、使用布尔字段容忍产品上不完整的 Theme、Anchor 失败时删除已成功导入的 Theme、用一个待补树 Theme 阻塞整个分析批次
...
通过 `POST /internal/data/v1/research-anchor-imports` 对一个已发布 Theme 的完整 Anchor 集合执行同步、原子、幂等发布的内部合同。顶层严格只有 `theme_id` 和 `anchors`；`theme_id` 同时表达父 Theme、整体发布边界和幂等身份。每棵 Anchor 严格只提交中心节点、独立结论、事实汇总、净方向、交易指向、下一检查点、Event 证据和有序路径节点。
...
通过 `GET /internal/data/v1/research/themes/{theme_id}/reasoning-trees` 返回某个 Theme 已发布的完整 Anchor Tab 摘要集合，并通过 `GET /internal/data/v1/research/themes/{theme_id}/reasoning-trees/{anchor_id}` 按需返回一棵完整推理树。一个 Theme 可以对应多个 Anchor，每个 Anchor 是一条独立推理树。列表不嵌入现有 Theme Detail，也不分页；详情必须校验 Anchor 属于路径中的 Theme。Theme 缺失、待补树或单树不存在分别使用明确 `404`，不返回合法空数组。
...
**Anchor 证据展示分组（Anchor Evidence Presentation Groups）**:
从 Anchor Event Evidence 角色直接得到的确定性页面分组：`driver` 和 `supporting` 属于当前支持，`contradicting` 属于当前反证，`context` 只属于背景事实。Data 和 BFF 保持单一 `events` 数组，小程序按角色过滤成展示区域。该分组不是新的研究判断，不得生成额外摘要、改变角色或重排 Event。
_Avoid_: 新增重复的支持/反证汇总字段、在 BFF 响应中复制 Event 数组、将 `context` 计入支持证据、无反证时隐藏区域或伪造反证

**Anchor 原子事件汇总（Anchor Atomic Event Rollup）**:
该 Research Anchor 关联的全部去重 Event 事实视图，覆盖 `driver`、`supporting`、`contradicting` 和 `context` 四种角色。它复用同一份 Anchor Event Evidence Association 和 Event 主数据，与支持/反证分组提供不同阅读视角，不创建第二套事实或关联。读取时按 `event_time` 从早到晚排列，时间相同时以 `event_id` 升序稳定排序，缺失时间的 Event 放在最后。
...
Research Anchor 主表只保存 Theme 归属、中心 Chain Node、发布回执和五个不可变自然语言快照字段：一句话结论、原子事实汇总、净方向、交易研究指向、下一检查点。它不复制 Theme 批次、节点名称、Theme 发布时间或旧 Anchor 分类字段；同一 Theme 与中心节点组合唯一。
...
AI 分析师 V3 与 Research Anchor Import V1 对 Anchor、Event Evidence 和 Path Node 使用同名结构。发布器只把 Theme Import 回执解析出的 `theme_id` 与对应完整 `anchors` 集合装入请求，不做语义转换。Tidewise 生成 Anchor/Receipt 身份与时间，并用 Chain Node、Event 主数据补充读取名称和展示字段；BFF 不改写研究语义。
