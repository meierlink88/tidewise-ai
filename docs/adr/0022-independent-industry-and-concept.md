...
- Data 使用独立的单数 `industry` 与 `concept` 表拥有正式事实，不引用 `entity_nodes`，不再
  使用 profile 表。
- 两类对象无损保留既有 `ENT + canonical lowercase UUID` ID；本次只把 owning 主键列从
  `entity_id` 改为 `id`，不执行身份重排。
...
Industry Chain 与 Chain Node ownership 不变；ADR-0016 的每种 Object 独立 Schema 决策继续
有效。
