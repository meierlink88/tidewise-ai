...
- `cloudwego/eino-ext/components/model/openai` at `v0.1.13`: inspected the constructor, custom Base URL support, structured response option, and examples. Adopted the official OpenAI-compatible `ChatModel` for DeepSeek, explicit Base URL/model/key/timeout mapping, an Execution-lifecycle-bound standard HTTP client, JSON object response format, and the `model.BaseChatModel` seam used by the Planner. Rejected a custom Provider protocol, Provider-specific types in the Collector domain model, and the former DeepSeek-specific component because its SDK pulled the unrelated vulnerable Ollama server module into AgentRun.
...
Issue #26 and `docs/specs/kratos-agentrun-service-migration.md` supersede this document's
original package-layout decision. The current package direction uses Kratos
`api/conf/biz/data/service/server` layers: reusable platform rules live in
`internal/biz/platform`, Collector and future Agent capabilities live in
`internal/biz/agents`, and PostgreSQL, Provider, Connector, Artifact and Scheduler
implementations live in `internal/data`. The Collector behavior frozen by this document
remains authoritative; only its engineering ownership was migrated.
...
- Non-secret runtime configuration is selected by `APP_ENV` and loaded from `configs/config.dev.yaml` or `configs/config.uat.yaml`. Environment variables are limited to the optional config directory override, database password or complete DSN, static inbound Service/Admin tokens, and the deployment-owned `TZ`.
