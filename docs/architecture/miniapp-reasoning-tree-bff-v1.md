...
`events` 保持唯一、完整、稳定排序的单一数组。BFF 不复制成支持/反证数组，也不重排；小程序按 `evidence_role` 确定性分组。`event_time` 使用 UTC RFC3339 或 `null`，路径顺序和 Tab 顺序由 Data 决定并原样保留。
...
5. 空反证数组、未量化路径说明和 `event_time: null` 不会被改写、删除或重排。
