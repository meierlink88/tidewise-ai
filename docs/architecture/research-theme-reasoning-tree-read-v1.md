...
```http
GET /internal/data/v1/research/themes/{theme_id}/reasoning-trees
...
```http
GET /internal/data/v1/research/themes/{theme_id}/reasoning-trees/{anchor_id}
Authorization: Bearer <service-token>
```
...
- 现有 `GET /internal/data/v1/research/themes` 完全不变，继续按 Theme 批次、查询窗口和游标分页返回首页内容。
...
- 现有 `GET /internal/data/v1/research/themes/{theme_id}` 合同也保持不变，不嵌入推理树。
...
- Miniapp `GET /api/v1/miniapp/research/anchors`。
- Miniapp `GET /api/v1/miniapp/research/anchors/{anchor_id}`。
