...
- HTTP 端点固定为 `POST /api/data/v1/research-anchor-imports`。
...
- Data 推理树列表端点固定为 `GET /api/data/v1/research/themes/{theme_id}/reasoning-trees`。
- Data 单树详情端点固定为 `GET /api/data/v1/research/themes/{theme_id}/reasoning-trees/{anchor_id}`。
- 两个端点均不接收 `window_hours` 或其他时间窗口参数；已发布推理树完全按路径中的 `theme_id` 读取，即使 Theme 已离开首页时间窗口仍可通过明确 ID 访问。
- 现有 `GET /api/data/v1/research/themes/{theme_id}` 合同保持不变，不在本期嵌入 Anchor 推理树。
...
- `GET /api/data/v1/research/themes` 只负责按现有 Theme 批次、时间窗口和分页合同返回 Theme，不查询或依赖 Research Anchor Publication Receipt。
...
- Miniapp BFF 新增 `GET /api/miniapp/v1/research/themes/{theme_id}/reasoning-trees`。
- Miniapp BFF 新增 `GET /api/miniapp/v1/research/themes/{theme_id}/reasoning-trees/{anchor_id}`。
...
- 旧 Miniapp `GET /api/miniapp/v1/research/anchors` 和 `GET /api/miniapp/v1/research/anchors/{anchor_id}` 不再保留。
