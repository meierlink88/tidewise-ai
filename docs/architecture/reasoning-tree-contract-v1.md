...
- Theme 本身作为技术暂存记录保留以支持 Anchor 重试，但在完整 Anchor 集合成功发布前不进入首页 Theme 列表。
...
### 30. 页面证据分组唯一由 evidence_role 决定

状态：已确认。

- `driver` 和 `supporting` Event 归入页面的“当前支持”。
- `contradicting` Event 归入页面的“当前反证”。
- `context` Event 只作为背景事实展示，不计入支持或反证。
- V1 不增加 `support_summary`、`counter_summary` 或其他与 Event `evidence_summary` 重复的 Anchor 汇总字段。
- Data Service 和 Miniapp BFF 都返回单一 `events` 数组及已发布的证据角色和说明；小程序按上述固定规则进行轻量过滤，但不生成、改写或重新判断证据语义。
- 没有 `contradicting` Event 时仍返回空数组，页面明确表达当前无反证，不隐藏该区域。
...
- 事实清单回答“本次研究使用了哪些原子事件”；当前支持和当前反证回答“这些事件在论证中承担什么作用”。
- 页面可在事实清单和证据分组中引用同一 Event，但数据库和 Data API 只维护一份 Anchor Event Evidence Association。
...
- 原子事件清单、当前支持、当前反证和背景事实必须保持该稳定顺序中的相对次序。
...
### 40. 首页只返回已完整发布推理树的 Theme

状态：已确认。

- 产品语义上，一个成立的 Theme 必须至少有一条推理树；由于 Theme Import V1 已要求至少一个 Theme Chain Node Association，完整 Anchor 覆盖成功后自然至少存在一条树。
- Theme Import V1 成功但 Anchor Import 尚未成功或失败时，Theme 行和其关联保留在 PostgreSQL 中以支持后续重试，但它只是技术暂存状态。
- `GET /internal/data/v1/research/themes` 必须只返回存在成功 Research Anchor Publication Receipt 的 Theme，不返回待补树 Theme。
- 因为列表中每个 Theme 都已保证拥有完整推理树集合，不增加 `has_reasoning_tree`。
- Anchor Import 失败不删除或回滚已成功的 Theme Import，也不允许任何部分 Anchor Tab 对产品可见。

### 41. 首页按单个 Theme 完整性展示，不等待整批 Anchor 就绪

状态：已确认。

- 同一 Theme Publication Batch 中，每个 Theme 是 Anchor 发布和产品可见性的独立边界。
- 若一个批次有三个 Theme，其中两个已成功发布完整 Anchor 集合而一个仍失败，首页立即展示已完成的两个。
- 一个 Theme 的 Anchor 失败不阻塞同批其他完整 Theme 变为产品可见。
- 首页 `theme_count` 与 `event_count` 只基于当前查询窗口内的产品可见 Theme 重新计算，Event 仍按 `event_id` 去重。
- Theme Import V1 的数据库整批原子性保持不变；本规则只定义后续 Anchor 就绪后的读取可见性。
...
- V1 响应不返回 Anchor Receipt 时间、`anchor_type`、`importance`、Anchor 级 `transmission_path`、指数或额外支持/反证汇总。
...
- 小程序使用 `evidence_role` 做确定性展示过滤：全部 Event 用于原子事实，`driver` + `supporting` 用于当前支持，`contradicting` 用于当前反证。
- 该过滤不改变顺序、角色或文本，不构成前端因果推理。
...
- `research_anchors` 只包含：`id`、`theme_id`、`center_chain_node_entity_id`、`import_receipt_id`、`one_line_conclusion`、`fact_summary`、`net_direction_summary`、`trading_direction`、`next_checkpoint`、`created_at`。
...
- 五个自然语言快照字段均必填并做非空白约束；Data Import 不生成默认文本。
...
| `events[].evidence_role` | `research_anchor_events.evidence_role` | 同名 | 支持、反证和背景分组 |
...
- 推理树列表请求收到 Theme 不存在或 Theme 尚未成功发布推理树两类 `404` 时，BFF 保留内部错误语义供日志诊断，小程序统一展示“该研究主题暂不可用”与返回操作。
