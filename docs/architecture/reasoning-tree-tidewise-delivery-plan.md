...
- 计划日期：2026-07-20
- 当前阶段：TW-01～TW-06 及 TW-07 前置修正已验收
- 当前解锁任务：TW-07 页面 PR Review
- 下一个门禁：TW-07 验收后进入 TW-08 独立链路验收
...
### TW-07 前置修正 Anchor 支持与反证汇总合同

**用途**：修正旧合同把具体 Event 分组直接当作“当前支持/当前反证”的语义错误，为 TW-07 提供 AI 生成的 Anchor 级结论字段。

**主要产物**：`support_summary`、可空 `counter_summary` 的 Import、数据库、Data、BFF 和共享 fixture 全链路；具体合同见 `research-anchor-branch-summaries-v1.md`。

**依赖**：TW-03、TW-04、TW-05 已验收，且现有 Anchor 数据可按发布前空表策略演进。

**验收**：有反证和无反证合同、幂等、原子事务、读取映射及相关回归测试全部通过。
...
| TW-07 前置修正 | 已确认 | 已完成 | 已通过 | 已验收 |
| TW-07 | 已确认 | 已完成 | 已通过 | 等待 PR Review |
