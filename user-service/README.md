...
1. 上线前核对小程序后台隐私保护指引中的手机号、昵称用途，与页面政策一致；确认手机号额度、
   API 域名及微信服务器 IP 白名单。政策正文在 miniapp/frontend/src/features/identity/privacy.ts。
2. 停止旧 User 服务后运行完整 Goose 账本到 schema 5。旧镜像 Ready 严格要求版本 4，
   因而此次不能宣称无停机滚动发布。不要修改已执行的 migration。
3. 启动新版 User（Ready 接受版本 >=5 并检测字段权限），再更新 Miniapp Backend，再发布前端。
4. 用真机测试一键登录、取消手机号授权、授权成功及两入口 user_id 一致；核对数据库手机号和
   会话同意版本，不在工单/日志输出号码或令牌。微信原生弹窗无法以 H5 替代验收。
5. 回退优先撤回新前端、保留新版兼容 User/BFF；不得直接回退旧版本4 User镜像。
   如必须回退 User 逻辑，应先制作适配 schema 5 Ready 的兼容回退镜像；保留新增列，不执行 down。

隐私政策为按当前功能制作的发布候选稿。正式上线前，运营主体应核对实际托管服务商、存储地域、
...
SELECT/INSERT/UPDATE/DELETE 权限（迁移与运行账号分离的环境）；然后升级 User、BFF 和
小程序。Ready 检查新表，未迁移时失败关闭。旧 schema5 应用可回退，保留头像表。
