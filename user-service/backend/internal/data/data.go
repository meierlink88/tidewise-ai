...
func Ready(ctx context.Context, db *sql.DB) error {
	var version int
	if err := db.QueryRowContext(ctx, `SELECT version_id FROM goose_db_version WHERE is_applied ORDER BY id DESC LIMIT 1`).Scan(&version); err != nil || version < 7 {
	...
	rows, err = db.QueryContext(ctx, `SELECT c.code,c.value,a.image_data,a.content_type,a.updated_at,a.privacy_version,w.stock_id,w.added_at FROM configuration c LEFT JOIN user_avatars a ON false LEFT JOIN user_watchlist w ON false LIMIT 0`)
	if err != nil {
