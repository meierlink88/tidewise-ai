...
func Ready(ctx context.Context, db *sql.DB) error {
	var version int
	if err := db.QueryRowContext(ctx, `SELECT version_id FROM goose_db_version WHERE is_applied ORDER BY id DESC LIMIT 1`).Scan(&version); err != nil || version != 3 {
	...
	rows, err = db.QueryContext(ctx, `SELECT code,value FROM user_configurations LIMIT 0`)
	if err != nil {
