...
迁移 `010` 只放宽既有状态约束。若候选版本验证失败且需恢复到仅包含 `001`–`010`
的旧 AgentRun，UAT 部署脚本会恢复旧版所需的 Journal Unit 唯一索引，并撤销已应用的
`011`–`014` ledger 标记。该操作保留向后兼容的新增表和注册数据；若已存在同一 Unit
的多条 Journal，则拒绝回滚，避免损坏数据。历史 Apply 一旦产生 `skipped`，不得再回滚到不认识
该状态的旧 AgentRun。
回滚标记会持久化上一发布的 AgentRun 迁移版本，因此若上一发布已拥有 `011`–`013`，恢复时会保留
这些 ledger，只撤销候选发布新增的迁移。
