...
func TestClientReadsTagCatalogAndPublishesExactBytes(t *testing.T) {
	...
	tags := []eventfact.Tag{{
		ID: "11111111-1111-4111-8111-111111111111", Kind: "news_category",
		Code: "technology", Name: "科技", IsActive: true,
	}}
	tagBytes, _ := json.Marshal(tags)
	tagSum := sha256.Sum256(tagBytes)
	tagHash := hex.EncodeToString(tagSum[:])
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case "/api/data/v1/event-tags":
			...
			_, _ = response.Write([]byte(`{"request_id":"data-1","result":{"catalog_revision":"event-tags:` + tagHash + `","catalog_hash":"` + tagHash + `","tags":[{"id":"11111111-1111-4111-8111-111111111111","tag_kind":"news_category","code":"technology","name":"科技","is_active":true}]}}`))
	...
	if catalog.Revision != "event-tags:"+tagHash || len(catalog.Tags) != 1 || catalog.Tags[0].Code != "technology" {
