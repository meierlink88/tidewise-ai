...
func (s *ResearchService) ListResearchThemeReasoningTrees(ctx context.Context, request *v1.ListResearchThemeReasoningTreesRequest) (*v1.ResearchReasoningTreeListResponse, error) {
	...
	for _, tree := range value.ReasoningTrees {
		trees = append(trees, v1.ResearchReasoningTreeSummary{ReasoningTreeID: tree.ReasoningTreeID, IndustryChainEntityID: tree.IndustryChainEntityID, IndustryChainName: tree.IndustryChainName, Title: tree.Title, DisplayOrder: tree.DisplayOrder, EventCount: tree.EventCount, PublishedAt: tree.PublishedAt})
	}
...
func themeItem(value biz.ResearchThemeItem) v1.ResearchThemeItem {
	...
	for _, impact := range value.Impacts {
		impacts = append(impacts, v1.ResearchThemeImpact{ChainNodeEntityID: impact.ChainNodeEntityID, Name: impact.Name, RelationRole: impact.RelationRole, ImpactDirection: impact.ImpactDirection, ImpactSummary: impact.ImpactSummary, PrimarySignalDisplaySummary: impact.PrimarySignalDisplaySummary, DisplayOrder: impact.DisplayOrder})
	}
...
func events(values []biz.ResearchEventDTO) []v1.ResearchEvent {
	...
	for _, e := range values {
		result = append(result, v1.ResearchEvent{EventID: e.EventID, Title: e.Title, Summary: e.Summary, EventTime: e.EventTime, EvidenceRole: e.EvidenceRole, SupportedClaim: e.SupportedClaim, DisplayOrder: e.DisplayOrder})
	}
	return result
}
func signal(value biz.ResearchSignalDTO) v1.ResearchReasoningTreeSignal {
	return v1.ResearchReasoningTreeSignal{VariableSignalKey: value.VariableSignalKey, SignalRole: value.SignalRole, SignalDirection: value.SignalDirection, DisplaySummary: value.DisplaySummary, DisplayOrder: value.DisplayOrder}
}
