...
func run(args []string, stdout, stderr io.Writer) int {
	...
	if *importTimeoutSeconds > 0 {
		importCtx, cancelImport = context.WithTimeout(context.Background(), time.Duration(*importTimeoutSeconds)*time.Second)
	} else {
		importCtx, cancelImport = context.WithCancel(context.Background())
	}
	defer cancelImport()
	for _, pkg := range packages {
		result, importErr := service.Import(importCtx, pkg)
		if importErr != nil {
			code := exitDatabase
