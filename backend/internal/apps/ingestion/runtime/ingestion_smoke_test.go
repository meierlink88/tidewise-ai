...
func TestNormalizeSmokeOptionsDefaultsConcurrencyToSerial(t *testing.T) {
	options := normalizeSmokeOptions(IngestionSmokeOptions{})
	if options.Concurrency != 1 {
		t.Fatalf("Concurrency = %d, want 1", options.Concurrency)
	}

	options = normalizeSmokeOptions(IngestionSmokeOptions{Concurrency: 4})
	if options.Concurrency != 4 {
		t.Fatalf("explicit Concurrency = %d, want 4", options.Concurrency)
	}
}
