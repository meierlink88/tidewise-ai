...
func (r IngestionSmokeRunner) Run(ctx context.Context, options IngestionSmokeOptions) (IngestionSmokeReport, error) {
	...
	registry.RegisterConnector("rss_feed", ingestionconnectors.RSSFeedConnector{Client: r.client})
	registry.RegisterParser("rss_item", ingestionparsers.RSSItemParser{})

	writer := &countingSmokeWriter{
		writer:       core.NewRawDocumentWriter(r.repository),
		maxDocuments: options.MaxDocuments,
	}
	job := NewIngestionJobWithOptions(
		core.NewSourceRegistry(r.repository),
		registry,
		core.EnvCredentialResolver{},
		core.NewRateLimiter(),
		writer,
		IngestionJobOptions{Concurrency: options.Concurrency},
	)
...
func (w *countingSmokeWriter) Write(ctx context.Context, candidate core.RawDocumentCandidate) (repositories.RawDocumentWriteResult, error) {
