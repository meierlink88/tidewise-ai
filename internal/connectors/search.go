...
func (c ParallelSearch) Collect(ctx context.Context, request collector.Request) ([]collector.Candidate, error) {
	if strings.TrimSpace(c.APIKey) == "" {
		return nil, fmt.Errorf("PARALLEL_API_KEY is missing")
	}
	endpoint := c.Endpoint
	if endpoint == "" {
...
func (c Tavily) Collect(ctx context.Context, request collector.Request) ([]collector.Candidate, error) {
	if strings.TrimSpace(c.APIKey) == "" {
		return nil, fmt.Errorf("TAVILY_API_KEY is missing")
	}
	endpoint := c.Endpoint
	if endpoint == "" {
	...
	query := strings.Join(request.SearchQueries, " ")
	...
	body := map[string]any{
		...
		"chunks_per_source": 3, "max_results": request.CandidateLimit,
	...
	results := make([]collector.Candidate, 0, len(response.Results))
	for _, item := range response.Results {
		...
		results = appendCandidate(results, collector.Candidate{
			...
			SourceName: host(item.URL), Content: content, ContentLevel: level, SourceType: "news",
		})
	}
	return results, nil
}
...
func (c Bocha) Collect(ctx context.Context, request collector.Request) ([]collector.Candidate, error) {
	if strings.TrimSpace(c.APIKey) == "" {
		return nil, fmt.Errorf("BOCHA_API_KEY is missing")
	}
	endpoint := c.Endpoint
	if endpoint == "" {
	...
	results := make([]collector.Candidate, 0, len(response.Data.WebPages.Value))
