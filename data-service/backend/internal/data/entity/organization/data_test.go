...
func TestStorePublishesCatalogAndPersistsOrganizationFacts(t *testing.T) {
	...
	if created.ID != "ORG_UN" || created.Category.NameZh != "政府间国际组织" || created.Function.NameZh != "安全与防务" || len(created.DomainTags) != 0 {
	...
	tagged, err := store.ReplaceDomainTags(ctx, "ORG_UN", []string{"REGIONAL_SECURITY_DIALOGUE"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := store.ReplaceDomainTags(ctx, "ORG_UN", []string{"AI_TECHNOLOGY_AND_GOVERNANCE"}); err == nil {
	...
	listed, err := store.List(ctx, organizationbiz.Filter{CategoryCode: "INTERGOVERNMENTAL", FunctionCode: "SECURITY", RegionID: "REG_GLOBAL"})
	if err != nil {
		t.Fatal(err)
	}
	if len(listed) != 1 || listed[0].ID != "ORG_UN" {
...
func TestStoreEnforcesMembershipHistoryAndClassifiesReferences(t *testing.T) {
	...
	first, err := store.CreateMember(ctx, organizationbiz.Member{
		OrganizationID: "ORG_UN", CountryID: "COU_CHN", MembershipType: "FULL_MEMBER", EffectiveDate: &effective2020,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := store.CreateMember(ctx, organizationbiz.Member{
		OrganizationID: "ORG_UN", CountryID: "COU_CHN", MembershipType: "OBSERVER", EffectiveDate: &effective2021,
	...
	if _, err := store.UpdateMember(ctx, "ORG_UN", first.ID, first); err != nil {
		t.Fatal(err)
	}
	...
	members, err := store.ListMembers(ctx, "ORG_UN", &asOf2020)
	if err != nil {
		t.Fatal(err)
	}
	...
	organizations, err := store.List(ctx, organizationbiz.Filter{CountryID: "COU_CHN", AsOfDate: &effective2021})
	if err != nil {
		t.Fatal(err)
	}
	if len(organizations) != 1 || organizations[0].ID != "ORG_UN" {
		t.Fatalf("Country membership filter = %#v", organizations)
	}
	if _, err := store.CreateMember(ctx, organizationbiz.Member{
		OrganizationID: "ORG_UN", CountryID: "COU_MISSING", MembershipType: "OBSERVER",
	}); err == nil {
...
func organizationFact() organizationbiz.Organization {
	...
	regionID, countryID := "REG_GLOBAL", "COU_CHN"
	return organizationbiz.Organization{
		ID: "ORG_UN", Code: "UN", Name: "联合国", NameEn: "United Nations", RegionID: &regionID,
...
VALUES('REG_GLOBAL','GLOBAL','全球','Global','GEOGRAPHIC');
