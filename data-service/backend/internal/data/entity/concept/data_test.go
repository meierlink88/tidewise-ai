...
func TestGetRejectsPersistedConceptOutsideBizVocabulary(t *testing.T) {
	...
	defer db.Close()
	now := time.Now().UTC()
	mock.ExpectQuery(`SELECT`).WithArgs(conceptbiz.ID("ENT33333333-3333-4333-8333-333333333333")).WillReturnRows(
		sqlmock.NewRows([]string{
			"id", "name", "aliases", "concept_type", "definition", "review_status", "created_at", "updated_at",
		}).AddRow(
			"ENT33333333-3333-4333-8333-333333333333", "人工智能", []byte(`["AI"]`), "sector", "错误概念", "candidate", now, now,
		),
	)
	...
	_, err = store.Get(context.Background(), conceptbiz.ID("ENT33333333-3333-4333-8333-333333333333"))
