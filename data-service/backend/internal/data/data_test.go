...
func TestPublishedObjectSchemasAndPersistenceStayAligned(t *testing.T) {
	...
	contracts := []objectSchemaContract{
		...
		{
			name:       "MacroEconomicDomain",
			schemaFile: "macro-economic-domain.schema",
			table:      "macro_economics_domain",
			schemaMembers: []string{
				"MacroEconomicDomain(宏观经济领域): EntityType",
				"code(领域代码): Text",
				"name(领域中文名称): Text",
				"description(领域描述): Text",
				"tactics(手段数组): Text",
			...
			constraints: []schemaConstraint{
				{name: "chk_macro_economics_domain_identity", requiredTokens: []string{"^MCD[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$"}},
				{name: "chk_macro_economics_domain_code", requiredTokens: []string{"^[A-Z][A-Z0-9_]{0,49}$"}},
				{name: "chk_macro_economics_domain_required_text", requiredTokens: []string{"btrim(name::text) <> ''::text", "btrim(description) <> ''::text"}},
				{name: "chk_macro_economics_domain_tactics", requiredTokens: []string{"validate_macroeconomic_domain_tactics(tactics)"}},
				{name: "chk_macro_economics_domain_timestamp_order", requiredTokens: []string{"updated_at >= created_at"}},
				{name: "macro_economics_domain_pkey", requiredTokens: []string{"PRIMARY KEY (id)"}},
				{name: "macro_economics_domain_code_key", requiredTokens: []string{"UNIQUE (code)"}},
				{name: "macro_economics_domain_name_key", requiredTokens: []string{"UNIQUE (name)"}},
			},
		},
		{
			name:       "MacroEconomic",
			...
			constraints: []schemaConstraint{
				...
				{name: "chk_macro_economics_timestamp_order", requiredTokens: []string{"updated_at >= created_at"}},
				{name: "macro_economics_pkey", requiredTokens: []string{"PRIMARY KEY (id)"}},
				{name: "macro_economics_name_key", requiredTokens: []string{"UNIQUE (name)"}},
				{name: "macro_economics_macro_economics_domain_id_fkey", requiredTokens: []string{"FOREIGN KEY (macro_economics_domain_id)", "REFERENCES macro_economics_domain(id)", "ON DELETE RESTRICT"}},
			},
		},
	}
