...
func TestMinistryAndInstitutionKindsUseReviewedPrefixes(t *testing.T) {
	for kind, expectedPrefix := range map[Kind]string{
		Ministry:    "MIN",
		Institution: "INS",
	} {
...
func TestStorylineKindsUseReviewedPrefixes(t *testing.T) {
	for kind, expectedPrefix := range map[Kind]string{
		Storyline:          "STL",
		StorylineEventLink: "SLE",
	} {
		value, err := New(kind)
		if err != nil {
			t.Fatalf("New(%q) error = %v", kind, err)
...
func TestCurrentEventKindsReplaceRetiredPublicationKinds(t *testing.T) {
	...
	for _, retired := range []Kind{"EER", "ETD", "ETA"} {
