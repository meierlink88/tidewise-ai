...
## Stock business fields (000095)

Issue #536：仅新增十个可空业务字段，无默认值、无数据回填，无新增来源/版本/财年字段。
现有 ID、自然键、基础数据、as_of 和时间戳不变；主营收入允许负数和非100合计，集合区分未知
NULL 与已知空数组。JSON 收入条目要求非空名称及数值 pct，字符串数组为无NULL元素的一维数组。
