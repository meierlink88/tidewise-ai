...
该命令顺序执行微信构建、产物门禁和固定目录发布。默认发布目录是 `~/Documents/WeChatProjects/tidewise-ai-preview`；本机实际路径为 `/Users/meierlink/Documents/WeChatProjects/tidewise-ai-preview`。如需覆盖，仅对当前命令设置 `TIDEWISE_WEAPP_PREVIEW_DIR`。旧 `/Users/meierlink/WeChatProjects/tidewise-ai-preview` 已迁移且不再更新，但暂时保留，避免已有开发者工具项目立即失效。
...
- 项目目录：`/Users/meierlink/Documents/WeChatProjects/tidewise-ai-preview`
- 不再导入任何主仓库或 worktree 的 `src/frontend/miniapp/dist`
...
4. 退出当前项目，重新选择“导入项目”，只选择新固定目录 `/Users/meierlink/Documents/WeChatProjects/tidewise-ai-preview`；不要再选择已停止更新的旧目录。
...
`src/frontend/miniapp/dist` 和 `~/Documents/WeChatProjects/tidewise-ai-preview` 都是本地生成目录，不提交 Git。发布器只替换目标目录内部内容，不删除目录外文件。本 change 仅验收微信小程序；既有抖音依赖与 `build:tt` 脚本保留，但不属于本次验收范围。

需要清理本地构建产物时，在仓库根目录执行：

```bash
find src/frontend/miniapp/dist -mindepth 1 -delete
```
