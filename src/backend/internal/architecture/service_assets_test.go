...
func TestServiceOwnedDockerAssetsReplaceLegacyBackendImage(t *testing.T) {
	...
	for _, asset := range assets {
		path := filepath.Join(repoRoot, "src", "backend", "services", asset.service, "Dockerfile")
	...
	if _, err := os.Stat(filepath.Join(repoRoot, "src", "backend", "Dockerfile")); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("legacy backend/Dockerfile must be removed after all local/CI consumers switch: %v", err)
	}
}

func TestProductSourceRootIsCanonical(t *testing.T) {
	repoRoot := filepath.Join("..", "..", "..", "..")
	for _, path := range []string{"src/backend", "src/frontend"} {
		info, err := os.Lstat(filepath.Join(repoRoot, filepath.FromSlash(path)))
		if err != nil {
			t.Fatalf("canonical product source %q is missing: %v", path, err)
		}
		if !info.IsDir() || info.Mode()&os.ModeSymlink != 0 {
			t.Fatalf("canonical product source %q must be a real directory", path)
		}
	}
	for _, path := range []string{"backend", "frontend"} {
...
func TestCIConsumesThreeServiceOwnedImagesAndBoundaryContracts(t *testing.T) {
	...
	for _, required := range []string{
		...
		"npm run build:weapp",
		"npm run build:admin",
		"docker build -f src/frontend/admin/Dockerfile -t tidewise-admin:ci .",
	} {
		if !strings.Contains(text, required) {
