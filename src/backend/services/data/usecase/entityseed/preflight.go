...
const phaseAPreflightMetricsSQL = `WITH metrics(metric, value) AS (
    ...
    UNION ALL SELECT 'catalog.legacy_function_reference', count(*)::bigint FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE p.prokind IN ('f','p') AND n.nspname NOT IN ('pg_catalog','information_schema') AND pg_get_functiondef(p.oid) ~* '(sector_profiles|sector_source_mappings|industry_chain_profiles|industry_chain_memberships|industry_chain_topology_edges|industry_chain_physical_constraints|entity_convergence)'
    UNION ALL SELECT 'catalog.legacy_trigger_reference', count(*)::bigint FROM pg_trigger t JOIN pg_class c ON c.oid=t.tgrelid WHERE NOT t.tgisinternal AND pg_get_triggerdef(t.oid) ~* '(sector_profiles|sector_source_mappings|industry_chain_profiles|industry_chain_memberships|industry_chain_topology_edges|industry_chain_physical_constraints|entity_convergence)'
    UNION ALL SELECT 'catalog.legacy_view_reference', count(*)::bigint FROM pg_views WHERE schemaname NOT IN ('pg_catalog','information_schema') AND definition ~* '(sector_profiles|sector_source_mappings|industry_chain_profiles|industry_chain_memberships|industry_chain_topology_edges|industry_chain_physical_constraints|entity_convergence)'
    UNION ALL SELECT 'catalog.legacy_rule_reference', count(*)::bigint FROM pg_rules WHERE schemaname NOT IN ('pg_catalog','information_schema') AND definition ~* '(sector_profiles|sector_source_mappings|industry_chain_profiles|industry_chain_memberships|industry_chain_topology_edges|industry_chain_physical_constraints|entity_convergence)'
)
