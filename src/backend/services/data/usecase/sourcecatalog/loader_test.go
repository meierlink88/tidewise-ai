...
func TestLoadManifestValidatesAIWebResearchSourceConfig(t *testing.T) {
		...
		"sources": [
			{
				...
				"source_config": {
					...
					"prompt_ref": "agent-run://prompts/ai-web-research/cn-finance-daily/v1",
					"prompt_version": "v1",
	...
	if source.SourceConfig["prompt_ref"] != "agent-run://prompts/ai-web-research/cn-finance-daily/v1" {
		t.Fatalf("prompt_ref = %v, want agent-run prompt reference", source.SourceConfig["prompt_ref"])
	}
}
