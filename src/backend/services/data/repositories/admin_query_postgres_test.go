...
func TestPostgresListRawDocumentsProjectsContentLevelInScannerOrder(t *testing.T) {
	...
	mock.ExpectQuery("(?s)SELECT id, source_id, ingest_channel, source_type, source_name, source_url,.*source_external_id, title, content_text, content_level, raw_object_uri, raw_mime_type,.*language, published_at, collected_at, content_hash, ingest_status.*FROM raw_documents").WithArgs("", "", "", 50, 0).WillReturnRows(sqlmock.NewRows([]string{
		...
		"11111111-1111-5111-8111-111111111111", "22222222-2222-5222-8222-222222222222", "rss", "news", "Example", "https://example.test/feed",
