...
```text
src/backend/data/entity_foundation/
```
...
cd src/backend
APP_ENV=local DATABASE_PASSWORD=<local-password> go run ./services/data/cmd/dbmigrate -apply
APP_ENV=local DATABASE_PASSWORD=<local-password> go run ./services/data/cmd/entity-seed
```

`services/data/cmd/entity-seed` 默认读取 `data/entity_foundation` 下的实体 seed 文件，并输出 JSON report。重复执行同一组 seed 应保持幂等，report 中应主要体现为 `unchanged`，不应新增重复实体、重复 profile 或重复关系。
...
```bash
cd src/backend
APP_ENV=local DATABASE_PASSWORD=<local-postgres-password> go run ./services/data/cmd/dbmigrate -apply
APP_ENV=local DATABASE_PASSWORD=<local-postgres-password> go run ./services/data/cmd/entity-seed
APP_ENV=local DATABASE_PASSWORD=<local-postgres-password> NEO4J_USERNAME=<local-neo4j-user> NEO4J_PASSWORD=<local-neo4j-password> go run ./services/data/cmd/graph-projector check
