...
bash -n "$neo4j_root/adopt-reason-provider.sh" "$neo4j_root/lib.sh" \
  "$neo4j_root/configure-office-access.sh" "$neo4j_root/test-rollback.sh" \
  "$neo4j_root/verify.sh"
bash "$neo4j_root/test-rollback.sh"

grep -qx 'server.bolt.listen_address=0.0.0.0:7687' "$neo4j_root/neo4j.conf.fragment"
grep -qx 'server.http.listen_address=0.0.0.0:7474' "$neo4j_root/neo4j.conf.fragment"
...
grep -q 'apply_neo4j_config_fragment' "$neo4j_root/configure-office-access.sh"
grep -q 'restore_neo4j_config' "$neo4j_root/configure-office-access.sh"
