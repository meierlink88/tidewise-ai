...
docker exec "$db" psql -U postgres -d tidewise_user_test -v ON_ERROR_STOP=1 -c \
  "CREATE ROLE user_runtime LOGIN PASSWORD 'user-runtime-test'; GRANT CONNECT ON DATABASE tidewise_user_test TO user_runtime; GRANT USAGE ON SCHEMA public TO user_runtime; GRANT SELECT,INSERT,UPDATE ON users,wechat_identities,user_sessions TO user_runtime; GRANT SELECT ON goose_db_version,user_configurations TO user_runtime;" >/dev/null
