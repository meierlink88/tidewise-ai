...
`frontend/miniapp/dist` 和 `~/Documents/WeChatProjects/tidewise-ai-preview` 都是本地生成目录，不提交 Git。发布器只替换目标目录内部内容，不删除目录外文件。本 change 仅验收微信小程序；既有抖音依赖与 `build:tt` 脚本保留，但不属于本次验收范围。
