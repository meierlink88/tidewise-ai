...
func TestProviderRuntimeHealthFixturesMatchAdminConsumer(t *testing.T) {
	fixtures := []struct {
		...
		{path: []string{"analyse-data-service", "backend", "api", "data", "v1", "testdata", "runtime-health.json"}, expected: []biz.RuntimeServiceKey{biz.RuntimeServiceData, biz.RuntimeServiceNeo4j}},
